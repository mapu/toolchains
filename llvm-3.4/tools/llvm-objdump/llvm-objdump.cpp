...
void llvm::DumpBytes(StringRef bytes) {
  ...
  #define MaxNumBytes 50
  enum { OutputSize = (MaxNumBytes * 3) + 1 };
  char output[OutputSize];

  // MaPU hacking: more than 16,41 bytes
  assert(bytes.size() <= MaxNumBytes
    && "DumpBytes only supports instructions of up to 50 bytes");
...
static void DisassembleObject(const ObjectFile *Obj, bool InlineRelocs) {
  ...
  for (section_iterator i = Obj->begin_sections(),
    ...
    //////////////////////// Fixme: MaPU SPU hacking ////////////////////
    // otherwise if we have symbols but none of them is at the section
    // begginning (with address 0), then we should insert a dummy symbol (".text")
    // to indicate section start.
    else if(Symbols[0].first != 0) {
      Symbols.insert(Symbols.begin(), std::make_pair(0, name));
    }
    /////////////////////// end MaPU SPU hacking ///////////////////////
