...
uint64_t ELFObjectWriter::SymbolValue(MCSymbolData &Data,
  ...
  /*// comment out by MaPU: we should distinguish Absolute and Variable
  ...
  */
  /////// Fixme: MaPU hacking: distinguish Absolute and Variable /////
  if(Symbol.isVariable()) {
    if(const MCExpr *Value = Symbol.getVariableValue()) {
      if(Symbol.isAbsolute()) {
        ...
        Value->EvaluateAsRelocatable(Res, Layout);
        const MCSymbolRefExpr * SRA = Res.getSymA();
        const MCSymbolRefExpr * SRB = Res.getSymB();
        if(SRA && !SRB) {
          MCSymbolData & SDA = Asm.getSymbolData(SRA->getSymbol());
          Data.setFragment(SDA.getFragment());
          Data.setOffset((int64_t) (SDA.getOffset()) + Res.getConstant());
        }
      }
    }
  }
...
void ELFObjectWriter::ComputeSymbolTable(MCAssembler &Asm,
  ...
  for (MCAssembler::symbol_iterator it = Asm.symbol_begin(),
    ...
    */
    ...
    {
      MSD.SectionIndex = ELF::SHN_ABS;
    } else if (RefSymbol.isVariable())
    {
      if(RefSymbol.isInSection())
      {
        const MCSectionELF &Section = static_cast<const MCSectionELF&>(RefSymbol.getSection());
        MSD.SectionIndex = SectionIndexMap.lookup(&Section);
        if (MSD.SectionIndex >= ELF::SHN_LORESERVE) NeedsSymtabShndx = true;
        assert(MSD.SectionIndex && "Invalid section index!");
      }
      else MSD.SectionIndex = ELF::SHN_ABS;
    /////////  MaPU Hacking  ////////////////
