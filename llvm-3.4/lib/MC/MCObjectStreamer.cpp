...
// MaPU hacking
void MCObjectStreamer::AddValueSymbolsForInsts(const MCInst &Inst) {
  for (unsigned i = Inst.getNumOperands(); i--; ) {

    if (Inst.getOperand(i).isExpr())
      AddValueSymbols(Inst.getOperand(i).getExpr());

    if (Inst.getOperand(i).isInst()) // perform this recursively.
      AddValueSymbolsForInsts(*Inst.getOperand(i).getInst());
  }
}
