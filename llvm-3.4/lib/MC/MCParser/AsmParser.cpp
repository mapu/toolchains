...
bool AsmParser::parseStatement(ParseStatementInfo &Info) {
  ...
  /************************** MaPU Hacking *********************************/
  const char * start = Lexer.getLoc().getPointer();

  Lexer.setCurPtr(start);
  StringRef line = Lexer.LexUntilEndOfLine();
  ...
  /************************** MaPU Hacking *********************************/
  if(getTargetParser().mnemonicIsValid(line, 0)) {
    IDVal = ID.getString();
    //Lex(); // We do not lex here.
    //ID = AsmToken(AsmToken::Target, IDVal);
  } else
  /************************** MaPU Hacking *********************************/
