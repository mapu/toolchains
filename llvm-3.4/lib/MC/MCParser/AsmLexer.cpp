...
AsmToken AsmLexer::LexDigit() {
  ...
  //if (*CurPtr == 'x') {
  if (*CurPtr == 'x' || *CurPtr == 'X') { // MaPU hacking: to support 0X...
    ++CurPtr;
    const char *NumStart = CurPtr;
...
AsmToken AsmLexer::LexToken() {
  ...
  case '+':
    if (*CurPtr == '=')
      return ++CurPtr, AsmToken(AsmToken::PlusEqual, StringRef(TokStart, 2));
    if (*CurPtr == '+')
  ...
  case ';':
    if(*CurPtr == ';')
      return ++CurPtr, AsmToken(AsmToken::DoubleSemicolon, StringRef(TokStart, 2));
    return AsmToken(AsmToken::Semicolon, StringRef(TokStart, 1));

  case '?': return AsmToken(AsmToken::Question, StringRef(TokStart, 1));
  ...
  case '$':
    if (*CurPtr == '$')
      return ++CurPtr, AsmToken(AsmToken::DollarDollar, StringRef(TokStart, 2));
