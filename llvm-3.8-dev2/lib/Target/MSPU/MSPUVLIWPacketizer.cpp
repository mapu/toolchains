...
public:
  // Ctor.
  MSPUPacketizerList(MachineFunction &MF, MachineLoopInfo &MLI, AliasAnalysis *AA,
                     const MachineBranchProbabilityInfo *MBPI);
...
bool MSPUPacketizer::runOnMachineFunction(MachineFunction &MF) {
  const TargetInstrInfo *TII = MF.getSubtarget().getInstrInfo();
  auto &MLI = getAnalysis<MachineLoopInfo>();
  auto *MBPI = &getAnalysis<MachineBranchProbabilityInfo>();
  auto *AA = &getAnalysis<AAResultsWrapperPass>().getAAResults();
  // Instantiate the packetizer.
  MSPUPacketizerList Packetizer(MF, MLI, AA, MBPI);
...
// ignorePseudoInstruction - Ignore bundling of pseudo instructions.
bool MSPUPacketizerList::ignorePseudoInstruction(const MachineInstr &MI,
...
bool MSPUPacketizerList::isSoloInstruction(const MachineInstr &MI) {
  if(MI.isInlineAsm())
    return true;
