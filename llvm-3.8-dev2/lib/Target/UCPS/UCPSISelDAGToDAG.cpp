//===-- UCPSISelDAGToDAG.cpp - A dag to dag inst selector for UCPS ------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the UCPS target.
//
//===----------------------------------------------------------------------===//
...
//===--------------------------------------------------------------------===//
/// UCPSDAGToDAGISel - UCPS specific code to select UCPS machine
/// instructions for SelectionDAG operations.
namespace
{
	class UCPSDAGToDAGISel: public SelectionDAGISel
	{
		private:
			/// Subtarget - Keep a pointer to the UCPS Subtarget around so that we can
			/// make the right decision when generating code for different targets.
			const UCPSSubtarget *Subtarget;
			UCPSTargetMachine& TM;
		public:
			explicit
			UCPSDAGToDAGISel(UCPSTargetMachine &_TM)
						: SelectionDAGISel(_TM), Subtarget(nullptr), TM(_TM)
			{
			}

			bool runOnMachineFunction(MachineFunction &MF) override {
			  Subtarget = &static_cast<const UCPSSubtarget &>(MF.getSubtarget());
			...
			SelectAddrJImm(SDValue Addr, SDValue &Base, SDValue &Offset)
			{
			  ...
			  if (Addr.getOpcode() == ISD::ADD || Addr.getOpcode() == ISD::OR) {
			    ...
			    if (CN) {
			      Offset = CurDAG->getTargetConstant(CN->getSExtValue(), SDLoc(Addr), getTargetLowering()->getPointerTy(CurDAG->getDataLayout()));
            ...
            return true;
			  ...
			  // if we only get frame index
				...
				if(FI) {
					Base = CurDAG->getTargetFrameIndex(FI->getIndex(), getTargetLowering()->getPointerTy(CurDAG->getDataLayout()));
					Offset = CurDAG->getTargetConstant(0, SDLoc(Addr), getTargetLowering()->getPointerTy(CurDAG->getDataLayout()));
					return true;
				}
        ...
        return true;
			...
			getPassName() const
			{
				return "UCPS DAG->DAG Pattern Instruction Selection";
			}

			// Include the pieces autogenerated from the target description.
			#include "UCPSGenDAGISel.inc"
      ...
      getFrameIndex(SDNode *N)
      {
        ...
        const UCPSFrameLowering *TFL =
         static_cast<const UCPSFrameLowering*>(TM.getFrameLowering());
			  unsigned offset = TFL->getFrameIndexOffset(*MF, FrameIndex);

			  DebugLoc dl = N->getDebugLoc();
			  SDValue offsetVal = CurDAG->getTargetConstant(offset, MVT::i32);
			  SDNode *NJ31 = CurDAG->getMachineNode(UCPSInst::AssignPtr, dl, MVT::i32, offsetVal);
			  SDValue op(NJ31,0);

			  SDValue FP = CurDAG->getRegister(UCPSReg::J28, MVT::i32);
			  SDNode *N1 = CurDAG->getMachineNode(UCPSInst::PtrAdd, dl, MVT::i32, FP, op);
        return CurDAG->SelectNodeTo(N1, UCPSInst::PtrAdd, MVT::i32, FP, op);
      }*/

			SDNode*
			getGlobalBaseReg()
			{
				/// getGlobalBaseReg() is defined in UCPSInstrInfo class.
				unsigned GlobalBaseReg =
				  MF->getInfo<UCPSMachineFunctionInfo>()->getGlobalBaseReg();
				/// TLI: TargetLowering
...
#include <iostream>
SDNode *UCPSDAGToDAGISel::Select(SDNode *N) {
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    ...
    case Intrinsic::ucps_callm:
    case Intrinsic::ucps_callmb:
      // DAG fix-up for CallM imm
      // The default DAG for intrinsic CallM is as follow:
      // (int_ucps_callm (XferAddr tglobaladdr))
      // which has to be change into:
      // (int_ucps_callm tglobaladdr)
      Opcode = IntNo == Intrinsic::ucps_callm ?
                   UCPSInst::__CallM : UCPSInst::__CallMB;
      if (N->getNumOperands() == 3
          && N->getOperand(2).getNode()->getOpcode() == UCPS_ISD::XferAddr) {
    ...
    case Intrinsic::ucps_loop:
      if (N->getNumOperands() == 4 &&
        ...
        if (isa<BlockAddressSDNode>(Xfer->getOperand(0).getNode())) {
          ...
          return CurDAG->getMachineNode(UCPSInst::LoopL0, SDLoc(N),
    ...
    /*		case ISD::SDIV:
     ...
     TopPart = SDValue(CurDAG->getMachineNode(UCPS::SRAri, dl, MVT::i32, DivLHS,
     CurDAG->getTargetConstant(31, MVT::i32)), 0);
     }
     else {
     TopPart = CurDAG->getRegister(UCPS::R21, MVT::i32);
     }
     ...
     TopPart = SDValue(CurDAG->getMachineNode(UCPS::WRYrr, dl, MVT::Glue, TopPart,
     CurDAG->getRegister(UCPS::R21, MVT::i32)), 0);

     // FIXME: Handle div by immediate.
     unsigned Opcode = N->getOpcode() == ISD::SDIV? UCPS::SDIVrr : UCPS::UDIVrr;
     ...
     unsigned Opcode = N->getOpcode() == ISD::MULHU? UCPS::UMULrr : UCPS::SMULrr;
     SDNode *Mul = CurDAG->getMachineNode(Opcode, dl, MVT::i32, MVT::Glue, MulLHS, MulRHS);
     // The high part is in the Y register.
     return CurDAG->SelectNodeTo(N, UCPS::RDY, MVT::i32, SDValue(Mul, 1));
     }*/
  }
...
/// inline asm expressions.
bool
UCPSDAGToDAGISel::SelectInlineAsmMemoryOperand(const SDValue &Op,
												char ConstraintCode,
...
/// createUCPSISelDag - This pass converts a legalized DAG into a
/// UCPS-specific DAG, ready for instruction scheduling.
///
FunctionPass *
llvm::createUCPSISelDag(UCPSTargetMachine &TM)
{
	return new UCPSDAGToDAGISel(TM);
}
