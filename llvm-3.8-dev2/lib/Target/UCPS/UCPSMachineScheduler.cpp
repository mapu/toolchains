//===- UCPSMachineScheduler.cpp - MI Scheduler for UCPS -------------===//
//
...
void UCPSVLIWScheduleMI::postprocessDAG() {
...
/// empirically.
bool UCPSResourceModel::isResourceAvailable(SUnit *SU) {
  if (!SU || !SU->getInstr())
    return false;
...
bool UCPSResourceModel::reserveResources(SUnit *SU) {
...
void UCPSVLIWScheduleMI::schedule() {
  DEBUG(dbgs()
...
void UCPSConvergingSched::initialize(ScheduleDAGMI *dag) {
  DAG = static_cast<UCPSVLIWScheduleMI*>(dag);
  ...
  Top.ResourceModel = new UCPSResourceModel(STI, DAG->getSchedModel());
  Bot.ResourceModel = new UCPSResourceModel(STI, DAG->getSchedModel());

  assert((!llvm::ForceTopDown || !llvm::ForceBottomUp) &&
         "-misched-topdown incompatible with -misched-bottomup");
}

void UCPSConvergingSched::releaseTopNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
void UCPSConvergingSched::releaseBottomNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
bool UCPSConvergingSched::SchedBoundary::checkHazard(SUnit *SU) {
...
void UCPSConvergingSched::SchedBoundary::releaseNode(SUnit *SU,
                                                     unsigned ReadyCycle) {
...
void UCPSConvergingSched::SchedBoundary::bumpCycle() {
...
void UCPSConvergingSched::SchedBoundary::bumpNode(SUnit *SU) {
  bool startNewCycle = false;
...
void UCPSConvergingSched::SchedBoundary::releasePending() {
...
void UCPSConvergingSched::SchedBoundary::removeReady(SUnit *SU) {
...
SUnit *UCPSConvergingSched::SchedBoundary::pickOnlyChoice() {
  if (CheckPending)
    releasePending();
...
#ifndef NDEBUG
void UCPSConvergingSched::traceCandidate(const char *Label,
                                             const ReadyQueue &Q,
...
int UCPSConvergingSched::SchedulingCost(ReadyQueue &Q, SUnit *SU,
...
UCPSConvergingSched::CandResult UCPSConvergingSched::
...
SUnit *UCPSConvergingSched::pickNodeBidrectional(bool &IsTopNode) {
...
SUnit *UCPSConvergingSched::pickNode(bool &IsTopNode) {
...
/// does.
void UCPSConvergingSched::schedNode(SUnit *SU, bool IsTopNode) {
  if (IsTopNode) {
