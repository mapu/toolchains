//===-- UCPSISelLowering.h - UCPS DAG Lowering Interface ------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that UCPS uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//

#ifndef UCPS_ISELLOWERING_H
#define UCPS_ISELLOWERING_H
...
namespace llvm {
namespace UCPS_ISD {    // UCPS Instruction Selection DAG
enum {
...
class UCPSTargetLowering: public TargetLowering {
private:
  const UCPSSubtarget &Subtarget;

public:
  UCPSTargetLowering(const TargetMachine &TM,
                     const UCPSSubtarget &STI);
  ...
  unsigned
  getSRetSize(const Function *CalleeFn, SelectionDAG &DAG) const;
};
}    // end namespace llvm

#endif    // UCPS_ISELLOWERING_H
