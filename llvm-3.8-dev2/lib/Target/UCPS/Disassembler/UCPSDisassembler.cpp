...
static bool UCPSTryAddingSymbolicOperand(int64_t Value, bool isBranch,
...
static DecodeStatus readInstruction(ArrayRef<uint8_t> Bytes,
                                    uint64_t address,
                                    uint64_t &numBytes,
                                    uint64_t &insnCodes,
                                    bool isLittleEndian) {
...
static unsigned
getUCPSRegister(const void *D, unsigned RC, unsigned RegNo)
{
...
{
	unsigned Reg = getUCPSRegister(Decoder, UCPSReg::RRegRegClassID, RegNo);
...
DecodeDRegRegisterClass(MCInst &Inst, unsigned RegNo,
		uint64_t Address, const void *Decoder)
{
	unsigned Reg = getUCPSRegister(Decoder, UCPSReg::DRegRegClassID, RegNo>>1);
	Inst.addOperand(MCOperand::createReg(Reg));
	return MCDisassembler::Success;
}
...
DecodeMCRegRegisterClass(MCInst &Inst, unsigned RegNo,
    uint64_t Address, const void *Decoder)
{
  unsigned Reg = getUCPSRegister(Decoder, UCPSReg::MCRegRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
DecodeJRegRegisterClass(MCInst &Inst, unsigned RegNo,
		uint64_t Address, const void *Decoder)
{
	unsigned Reg = getUCPSRegister(Decoder, UCPSReg::JRegRegClassID, RegNo);
	Inst.addOperand(MCOperand::createReg(Reg));
	return MCDisassembler::Success;
}
...
DecodeKMRegRegisterClass(MCInst &Inst, unsigned RegNo,
		uint64_t Address, const void *Decoder)
{
	unsigned Reg = getUCPSRegister(Decoder, UCPSReg::KMRegRegClassID, RegNo);
	Inst.addOperand(MCOperand::createReg(Reg));
	return MCDisassembler::Success;
}
...
DecodeI32RegRegisterClass(MCInst &Inst, unsigned RegNo,
		uint64_t Address, const void *Decoder)
{
	unsigned Reg = getUCPSRegister(Decoder, UCPSReg::I32RegRegClassID, RegNo);
	Inst.addOperand(MCOperand::createReg(Reg));
	return MCDisassembler::Success;
}
...
DecodePtrRegRegisterClass(MCInst &Inst, unsigned RegNo,
		uint64_t Address, const void *Decoder)
{
	unsigned Reg = getUCPSRegister(Decoder, UCPSReg::PtrRegRegClassID, RegNo);
	Inst.addOperand(MCOperand::createReg(Reg));
	return MCDisassembler::Success;
}
...
DecodeSVRRegRegisterClass(MCInst &Inst, unsigned RegNo,
		uint64_t Address, const void *Decoder)
{
	unsigned Reg = getUCPSRegister(Decoder, UCPSReg::SVRRegRegClassID, RegNo);
	Inst.addOperand(MCOperand::createReg(Reg));
	return MCDisassembler::Success;
}
...
DecodeSHURegRegisterClass(MCInst &Inst, unsigned RegNo,
		uint64_t Address, const void *Decoder)
{
	unsigned Reg = getUCPSRegister(Decoder, UCPSReg::SHURegRegClassID, RegNo);
	Inst.addOperand(MCOperand::createReg(Reg));
	return MCDisassembler::Success;
}
...
DecodeTRegRegisterClass(MCInst &Inst, unsigned RegNo,
		uint64_t Address, const void *Decoder)
{
	unsigned Reg = getUCPSRegister(Decoder, UCPSReg::TRegRegClassID, RegNo);
	Inst.addOperand(MCOperand::createReg(Reg));
	return MCDisassembler::Success;
}
...
DecodeAddrJJ(MCInst &Inst, InsnType bits, uint64_t Address, const void *Decoder) {
  unsigned reg0 = bits & 0x1F;
	...
	unsigned RegEnum0  = getUCPSRegister(Decoder, UCPSReg::PtrRegRegClassID, reg0);
  unsigned RegEnum1  = getUCPSRegister(Decoder, UCPSReg::PtrRegRegClassID, reg1);
...
DecodeAddrJImm(MCInst &Inst, InsnType bits, uint64_t Address, const void *Decoder) {
  ...
  unsigned RegEnum  = getUCPSRegister(Decoder, UCPSReg::PtrRegRegClassID, BaseRegNo);
...
DecodeRJReg(MCInst &Inst, InsnType bits, uint64_t Address, const void *Decoder) {
  ...
  unsigned RegEnum;
  if(bits & (1<<5)) // is JReg
  	RegEnum	= getUCPSRegister(Decoder, UCPSReg::JRegRegClassID, reg);
  else
  	RegEnum	= getUCPSRegister(Decoder, UCPSReg::RRegRegClassID, reg);
...
DecodeRDReg(MCInst &Inst, InsnType bits, uint64_t Address, const void *Decoder) {
  ...
  if(bits & (1<<5)) // is DReg, otherwise RReg
  	RegEnum	= getUCPSRegister(Decoder, UCPSReg::DRegRegClassID, reg>>1);
  else
  	RegEnum	= getUCPSRegister(Decoder, UCPSReg::RRegRegClassID, reg);
...
DecodeKReg(MCInst &Inst, InsnType bits, uint64_t Address, const void *Decoder)
{
	...
	unsigned rn =0;
	switch(bits >>4) {
		case 0: rn = getUCPSRegister(Decoder, UCPSReg::KBRegRegClassID, reg); break;
		case 1: rn = getUCPSRegister(Decoder, UCPSReg::KERegRegClassID, reg); break;
		case 2: rn = getUCPSRegister(Decoder, UCPSReg::KSRegRegClassID, reg); break;
		case 3: rn = getUCPSRegister(Decoder, UCPSReg::KIRegRegClassID, reg); break;
		case 5:
			reg >>= 2;
			rn = getUCPSRegister(Decoder, UCPSReg::KGRegRegClassID, reg); break;
		case 6:
			reg >>= 2;
			rn = getUCPSRegister(Decoder, UCPSReg::KLRegRegClassID, reg); break;
		default:
...
DecodeLdStReg(MCInst &Inst, InsnType bits, uint64_t Address, const void *Decoder)
{
	...
	unsigned rn =0;
	switch(bits >>5) {
		case 0: rn = getUCPSRegister(Decoder, UCPSReg::RRegRegClassID, reg); break;
    case 1: rn = getUCPSRegister(Decoder, UCPSReg::DRegRegClassID, reg>>1); break;
		case 2:
		case 3: rn = getUCPSRegister(Decoder, UCPSReg::JRegRegClassID, reg); break;
		default:
...
static DecodeStatus
DecodeSImm17(MCInst &Inst, uint64_t bits, uint64_t Address, const void *Decoder) {
  const UCPSDisassembler *Dis = static_cast<const UCPSDisassembler*>(Decoder);
  ...
  if( !UCPSTryAddingSymbolicOperand(imm       /* imm value */,
                                    true      /* isBranch */,
...
// decode into *Inst*
DecodeStatus UCPSDisassembler::getInstruction(MCInst &Inst, uint64_t &Size,
                                              ArrayRef<uint8_t> Bytes, uint64_t Address, raw_ostream &vStream,
		raw_ostream &cStream) const
{
  ...
  do {
    ...
    result = decodeInstruction(DecoderTableUCPSDecode32, *ITR, codes & (~(1 << 31)), Address+Size, this, STI);
    if (result != MCDisassembler::Success) {
      delete ITR;
      return MCDisassembler::Success;
    }
...
extern Target llvm::TheUCPSTarget;

static MCDisassembler *createUCPSDisassembler(const Target &T, const MCSubtargetInfo &STI, MCContext &Ctx) {
  return new UCPSDisassembler(STI, Ctx, T.createMCRegInfo(""), true);
}
