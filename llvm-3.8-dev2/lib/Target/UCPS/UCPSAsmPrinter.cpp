//===-- UCPSAsmPrinter.cpp - Print machine instrs to UCPS assembly --===//
//
...
// of machine-dependent LLVM code to UCPS assembly language. This printer is
...
void UCPSAsmPrinter::EmitEndOfAsmFile(Module &M)
{
...
void UCPSAsmPrinter::EmitStartOfAsmFile(Module &M) {
...
//
bool
UCPSAsmPrinter::isBlockOnlyReachableByFallthrough(const MachineBasicBlock *MBB) const
{
...
void UCPSAsmPrinter::LowerToMC(const MachineInstr* MI, MCInst& MCI, MCInst *Head) {
...
/// printMachineInstruction -- Print out a single UCPS MI to
/// the current output stream.
/// Note: Caller of this function uses bundle iterator.
void UCPSAsmPrinter::EmitInstruction(const MachineInstr *MI) {
  if (MI->isBundle()) {
...
extern "C" void LLVMInitializeUCPSAsmPrinter() {
  RegisterAsmPrinter<UCPSAsmPrinter> X(TheUCPSTarget);
}
