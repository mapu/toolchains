...
private:
  ...
  public:
    ...
    inline void local(Symbol_table* symtab, Layout* layout, Target_mspu* target,
        Sized_relobj_file<size, big_endian>* object, unsigned int data_shndx,
        Output_section* output_section,
...
private:
  ...
  public:
    ...
    inline bool relocate(const Relocate_info<size, big_endian>*, Target_mspu*,
...
Target::Target_info Target_mspu<32, false>::mspu_info = {
  ...
  ".MSPU.attributes",  // attributes_section
  "mspuabi",    // attributes_vendor
...
public:
  Target_selector_mspu() :
      Target_selector(elfcpp::EM_NONE, size, big_endian, "elf32-mspu",
          "elf32mspu") {
  }
...
public:
  ...
  template<int valsize> static inline void mmpu_rela(unsigned char* view,
      unsigned int right_shift, unsigned int left_shift,
      elfcpp::Elf_Xword dst_mask,
      const Sized_relobj_file<size, big_endian>* object,
      const Symbol_value<size>* psymval,
...
template<int size, bool big_endian>
void Target_mspu<size, big_endian>::gc_process_relocs(Symbol_table* symtab,
  ...
  gold::gc_process_relocs<size, big_endian, Mspu, elfcpp::SHT_RELA, Scan,
      typename Target_mspu::Relocatable_size_for_reloc>(symtab, layout, this,
...
template<int size, bool big_endian>
void Target_mspu<size, big_endian>::scan_relocs(Symbol_table* symtab,
    Layout* layout, Sized_relobj_file<size, big_endian>* object,
    unsigned int data_shndx, unsigned int sh_type, const unsigned char* prelocs,
    size_t reloc_count, Output_section* output_section,
    bool needs_special_offset_handling, size_t local_symbol_count,
  ...
  gold::scan_relocs<size, big_endian, Mspu, elfcpp::SHT_RELA, Scan>(symtab,
...
void Target_mspu<size, big_endian>::relocate_section(
  ...
  gold::relocate_section<size, big_endian, Mspu, elfcpp::SHT_RELA,
                         Mspu_relocate, gold::Default_comdat_behavior>(
...
/****************************************************************/
template<int size, bool big_endian>
void Target_mspu<size, big_endian>::relocate_relocs(
    const Relocate_info<size, big_endian>* relinfo, unsigned int sh_type,
    const unsigned char* prelocs, size_t reloc_count,
    Output_section* output_section,
    typename elfcpp::Elf_types<size>::Elf_Off offset_in_output_section,
...
template<int size, bool big_endian>
inline void Target_mspu<size, big_endian>::Scan::local(Symbol_table* symtab,
    Layout* layout, Target_mspu* target,
    Sized_relobj_file<size, big_endian>* object, unsigned int data_shndx,
...
// GOT or share libraries in Powerpc.cc
template<int size, bool big_endian>
inline void Target_mspu<size, big_endian>::Scan::global(Symbol_table* symtab,
    Layout* layout, Target_mspu* target,
    Sized_relobj_file<size, big_endian>* object, unsigned int data_shndx,
    Output_section* output_section, const elfcpp::Rela<size, big_endian>& reloc,
...
/**********************************************************************/
template<int size, bool big_endian> inline bool Target_mspu<size, big_endian>::Relocate::relocate(
    const Relocate_info<size, big_endian>* relinfo, Target_mspu* target,
  ...
  case elfcpp::R_MSPU_17:   //This is the CallM instruction
    Reloc::template mmpu_rela<32>(view, 0, 5, ((1 << 17) - 1) << 5, object,
        psymval, addend);
    break;
