...
static void mapudump(char* p, size_t len)
{
...
Communication::Write (const void *src, size_t src_len, ConnectionStatus &status, Error *error_ptr)
{
    ...
    if (connection_sp.get())
    {
      size_t sz = connection_sp->Write (src, src_len, status, error_ptr);

      // MaPU hacking.
      /*if(sz>0 && src) {
        Log *log(lldb_private::GetLogIfAllCategoriesSet (LIBLLDB_LOG_COMMUNICATION));

        char* p = strndup((const char*)src, sz);
        if(p) {
          log->Printf("send %s (%d)", p, sz);
          delete p;
        }
      }*/
