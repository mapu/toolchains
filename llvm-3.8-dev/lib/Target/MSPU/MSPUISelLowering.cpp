                                ...
                                SDLoc DL, SelectionDAG &DAG) const
{
  ...
  if (MF.getFunction()->hasStructRetAttr()) {
    ...
    RetOps.push_back(DAG.getRegister(MSPUReg::R0, DAG.getTargetLoweringInfo().getPointerTy(DAG.getDataLayout())));
  }
...
unsigned
MSPUTargetLowering::getSRetSize(const Function* CalleeFn, SelectionDAG &DAG) const
                                {
...
MSPUTargetLowering::MSPUTargetLowering(const TargetMachine &TM, const MSPUSubtarget &STI)
...
MSPUTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const
{
	  ...
	  return DAG.getNode(MSPU_ISD::XferAddr, DL, DAG.getTargetLoweringInfo().getPointerTy(DAG.getDataLayout()), v);
}
...
LowerBR_CC(SDValue Op, SelectionDAG &DAG)
{
	...
	return DAG.getNode(Opc, dl, MVT::Other, Chain, Dest, DAG.getConstant(MSPU_CC, dl, MVT::i32), CompareFlag);
}
...
LowerSELECT_CC(SDValue Op, SelectionDAG &DAG)
{
	...
	return DAG.getNode(Opc, dl, TrueVal.getValueType(), TrueVal, FalseVal, DAG.getConstant(MSPU_CC, dl, MVT::i32),
						CompareFlag);
}
...
MSPUTargetLowering::getConstraintType(StringRef Constraint) const
{
...
MSPUTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
