...
public:
  UCPSAsmParser(const MCSubtargetInfo &_STI, MCAsmParser &_Parser,
    ...
    cachedLabel = false;
    Scheduler = new UCPSScheduler(Parser);
  }
...
#include "UCPSGenInstrFLexer.flex"

/* error handling for bison generated ucpsparse() */
void ucpserror(YYLTYPE *Loc,
...
MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode, OperandVector &Operands,
  ...
  std::shared_ptr<UCPSAsmOperand> op;
  MCOperand MO;
  ...
  do {
    for(unsigned i=0; ops && i < ops->size(); i++) {
      op = std::static_pointer_cast<UCPSAsmOperand>((*ops)[i]);
      switch(op->getKind()) {
      default:
        return false;

      case AsmOpcode:
        if (op->getOpc() == UCPS::NOP) break;
        NewInst = new MCInst;
  ...
  if (Inst) {
    if (OptContext.mseq != UCPS::NOP) {
      ...
      CurInst->addOperand(MO);
      Inst->addOperand(MO);
      if (OptContext.mseq == UCPS::LPTO) {
        MO = MCOperand::createExpr(OptContext.label);
        IP->addOperand(MO);
        Inst->addOperand(MO);
      }
      if (OptContext.mseq == UCPS::LPTO || OptContext.mseq == UCPS::REPEATK) {
...
ParseInstruction(ParseInstructionInfo &Info, StringRef Name, SMLoc NameLoc,
                 OperandVector &Operands) {
  YY_BUFFER_STATE buf = ucps_create_buffer(0, YY_BUF_SIZE);
  ucps_switch_to_buffer(buf);
  ucps_flush_buffer(buf);
  CurLoc = NameLoc;
  TokStart = NameLoc;  // Used for tracing the start location of each token

  BEGIN INITIAL;
  int err =  ucpsparse(Operands);

  ucps_delete_buffer(buf);
...
bool UCPS::UCPSAsmParser::ParseDirectiveHMacro(AsmToken DirectiveID,
                                                       SMLoc DirectiveLoc) {
  StringRef Name;
  if (inHMacro)
...
bool UCPS::UCPSAsmParser::ParseDirectiveEndHMacro(AsmToken DirectiveID,
                                                          SMLoc DirectiveLoc) {
  if (!inHMacro)
...
bool UCPS::UCPSAsmParser::ParseDirectiveOpt(AsmToken DirectiveID,
                                                    SMLoc DirectiveLoc) {
  EnableOptimization = true;
  return false;
}

bool UCPS::UCPSAsmParser::ParseDirectiveStopOpt(AsmToken DirectiveID,
                                                        SMLoc DirectiveLoc) {
  EnableOptimization = false;
  SMLoc Loc;
  EmitCachedInst(Loc, Parser.getStreamer(), 0);
...
bool UCPS::UCPSAsmParser::HandleHMacroEntry(StringRef Name,
                                                    SMLoc NameLoc,
                                                    const HMacro *M,
                                                    StringRef Prefix) {
 ...
 */
void UCPS::UCPSAsmParser::onLabelParsed(MCSymbol *Symbol) {
 ...
 */
int UCPS::UCPSAsmParser::AnalyzeHMacro(unsigned &Sym) {
  MCLoopBlock* MB;
  MCBlockTy PreType = UCPS::None, CurType;
  int64_t MaxCnt = 0;
  ...
  for (unsigned i = 0; i < ActiveHMacros.size(); i++) {
    if (ActiveHMacros[i]->getContainer(VAddr, MB)) {
      ...
      if ((CurType == Sequential && PreType == Repeat) ||
        ...
        else if (PreType == UCPS::None)
          MaxCnt = MB->getCount();
        else // loop with repeat is illegal
          return -1;
      }
      if (CurType != UCPS::None) PreType = CurType;
    }
  ...
  for (unsigned i = 0; i < ActiveHMacros.size(); i++) {
    if (ActiveHMacros[i]->getContainer(VAddr + 1, MB) || 
      ...
      if (MB->getType(VAddr + 1) != UCPS::None &&
...
void UCPS::UCPSAsmParser::DealWithSequential() {
...
void UCPS::UCPSAsmParser::DealWithRepeat(int64_t loops) {
  MCLoopBlock *MB, *NewMB;
  if (loops <= 0) { // insts are all repeat KI
    ...
    OptContext.mseq = UCPS::REPEATIMM;
    OptContext.label = NULL;
    OptContext.ki = 0;
    OptContext.count = loops;
  }
...
void UCPS::UCPSAsmParser::DealWithLoop(int64_t insts, bool Mode) {
  MCLoopBlock *MB;
  if (insts > 0) {
    for (unsigned i = 0; i < ActiveHMacros.size(); i++)
      if (ActiveHMacros[i]->getContainer(VAddr, MB) || 
...
bool UCPS::UCPSAsmParser::DealWithUnmatchedLoop(uint64_t KI,
                                                unsigned Level,
                                                int64_t &MaxCnt) {
  MCLoopBlock *MB;
  MCBlockTy PreType = UCPS::None;
  MaxCnt = 0;
  for (unsigned i = 0; i < ActiveHMacros.size(); i++) {
    if (ActiveHMacros[i]->getContainer(VAddr, MB) ||
        ActiveHMacros[i]->getNextBlock(VAddr, MB)) {
...
void UCPS::UCPSAsmParser::tryMergeRedundantInst(MCInst* &Inst,
                                                MCInst* &CurInst) {
  bool identical = true;
  ...
  if (OptContext.mseq == UCPS::LPTO || OptContext._mseq == UCPS::LPTO)
    identical = false;
  else if (OptContext.mseq == UCPS::REPEATK &&
           OptContext._mseq == UCPS::REPEATK)
    identical = false;
  else if (OptContext.mseq == UCPS::REPEATK) {
    if (OptContext.count == 0) identical = false;
    else if (OptContext._mseq == UCPS::REPEATIMM &&
             OptContext._count > -OptContext.count)
      identical = false;
  }
  else if (OptContext._mseq == UCPS::REPEATK) {
    if (OptContext._count == 0) identical = false;
    else if (OptContext.mseq == UCPS::REPEATIMM &&
  ...
  if (OptContext._mseq == UCPS::NOP) {
    if (OptContext.mseq == UCPS::NOP) {
      OptContext._mseq = UCPS::REPEATIMM;
      OptContext._count = 2;
    }
    else {
      OptContext._mseq = OptContext.mseq;
...
void UCPS::UCPSAsmParser::EmitCachedInst(SMLoc IDLoc, MCStreamer &Out, unsigned Sym) {
  ...
  if (OptContext._mseq != UCPS::NOP) {
    ...
    CurInst->addOperand(MO);
    Inst->addOperand(MO);
    if (OptContext._mseq == UCPS::LPTO) {
      MO = MCOperand::createExpr(OptContext._label);
      IP->addOperand(MO);
      Inst->addOperand(MO);
    }
    if (OptContext._mseq == UCPS::LPTO || OptContext._mseq == UCPS::REPEATK) {
  ...
  OptContext._mseq = UCPS::NOP;
  OptContext._label = NULL;
  OptContext._ki = 0;
  OptContext._count = 0;
}

void UCPS::UCPSAsmParser::HMacroDump() {
  raw_ostream &OS = errs();
...
void UCPS::UCPSAsmParser::RenameHMacro(MCParsedInst *PI) {
  for(unsigned i= 0; i < PI->size(); ++i) {
    std::shared_ptr<UCPS::UCPSAsmOperand> op =
      std::static_pointer_cast<UCPS::UCPSAsmOperand>(PI->getOperand(i));
    if (op->isHMacro()) {
...
extern "C"
void
LLVMInitializeUCPSAsmParser() {
  RegisterMCAsmParser<UCPS::UCPSAsmParser> X(TheUCPSTarget);
}
/*
