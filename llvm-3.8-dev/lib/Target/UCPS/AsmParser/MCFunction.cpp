...
bool MCParsedInst::hasRepeat(unsigned &OpNo) const {
  for(OpNo = 0; OpNo < Operands.size(); OpNo++) {
    std::shared_ptr<UCPS::UCPSAsmOperand> op =
      std::static_pointer_cast<UCPS::UCPSAsmOperand>(Operands[OpNo]);
    if (op->isToken() && 
        (op->getOpc() == UCPS::REPEATIMM ||
         op->getOpc() == UCPS::REPEATK))
      return true;
  }
  return false;
}
...
bool MCParsedInst::hasLoop(unsigned &OpNo) const {
  for(OpNo = 0; OpNo < Operands.size(); OpNo++) {
    std::shared_ptr<UCPS::UCPSAsmOperand> op =
      std::static_pointer_cast<UCPS::UCPSAsmOperand>(Operands[OpNo]);
    if (op->isToken() && op->getOpc() == UCPS::LPTO) return true;
  }
  return false;
}
...
bool MCParsedInst::hasEmbeddedHMacro(unsigned &OpNo) const {
  for(OpNo = 0; OpNo < Operands.size(); OpNo++) {
    ...
    if (op->isHMacro() && (OpNo + 1 < Operands.size())) {
      op = std::static_pointer_cast<UCPS::UCPSAsmOperand>(Operands[OpNo + 1]);
...
void MCLoopBlock::ExpandEmbeddedHM(StringMap<HMacro*> &Map) {
  ...
  while (!EmbeddedHMs.empty()) {
    ...
    if (start > Start) {
      ...
      Nops.push_back(SharedOperand(UCPSAsmOperand::createOpc(NOP)));
      if (start - Start == 1) {
...
void MCFunction::addParsedInst(MCParsedInst *Inst) {
  ...
  if (cachedLabel) {
    PendingLabels.push_back(
      std::pair<std::shared_ptr<UCPS::UCPSAsmOperand>, bool>(cachedLabel, false));
    cachedLabel = nullptr;
    while (PreviousInst && PreviousInst->hasLoop(OpNo)) {
      ...
      if (op->isExpr()) {
        ...
        if (CE) return; // absolute address is ignored
          ...
          if (SE) {
            PendingLabels.push_back(
              std::pair<std::shared_ptr<UCPS::UCPSAsmOperand>, bool>(op, false));
  ...
  if (Inst->hasRepeat(OpNo)) {
    ...
    if (op->getOpc() == UCPS::REPEATIMM && count == 0) count = 1024;
    else if (op->getOpc() == UCPS::REPEATK) {
      ki = count;
