...
extern struct target_type mapu_spu_target_type;
static struct target_type *target_types[] =
{
	&mapu_spu_target_type,
	NULL,
};
...
int target_finish(struct target *target)
{
	int retval;
	if (!target_was_examined(target))
	{
	...
	retval = target->type->spu_finish(target);
	if (retval != ERROR_OK)
		return retval;
...
static int target_write_buffer_default(struct target *target, uint32_t address, uint32_t size, const uint8_t *buffer)
{
  ...
  uint32_t old_val = 0;
	...
	if (address % 4)
	{
		...
		uint32_t aligned_address = address & ~3UL;

		if (unaligned > size)
			unaligned = size;

    target_read_memory(target, aligned_address, 4, 1, (uint8_t *)&old_val);

    for (unsigned i = 0; i < unaligned; i++)
      ((uint8_t *)&old_val)[(address & 3UL) + i] = buffer[i];

		if ((retval = target_write_memory(target, aligned_address, 4, 1, (uint8_t *)&old_val)) != ERROR_OK)
			return retval;
  ...
  if (size > 0)
  {
    target_read_memory(target, (address & ~3UL), 4, 1, (uint8_t *)&old_val);
    for (unsigned i = 0; i < size; i++)
      ((uint8_t *)&old_val)[i] = buffer[i];
    return target_write_memory(target, address, 4, 1, (uint8_t *)&old_val);
  }
...
static int target_read_buffer_default(struct target *target, uint32_t address, uint32_t size, uint8_t *buffer)
{
  ...
  uint32_t aligned_val = 0;
    ...
    uint32_t aligned_address = address & ~3UL;
		...
		if ((retval = target_read_memory(target, aligned_address, 4, 1, (uint8_t *)&aligned_val)) != ERROR_OK)
			return retval;

		for (unsigned i = 0; i < unaligned; i++)
		  buffer[i] = ((uint8_t *)&aligned_val)[(address & 3UL) + i];
  ...
  if (size >= 4)
  {
	...
	if (size > 0)
	{
		if ((retval = target_read_memory(target, address, 4, 1, (uint8_t *)&aligned_val)) != ERROR_OK)
			return retval;
		for (unsigned i = 0; i < size; i++)
		  buffer[i] = ((uint8_t *)&aligned_val)[i];
	}
 ...
 * You must FREE the memory allocated within. */
static char *target_name_ext(struct target *target, char * name_ext)
{
	if(target->virt)
		sprintf(name_ext, "%s(mask 0x%x)", target->cmd_name, target->spu_mask);
	else
		sprintf(name_ext, "%s", target->cmd_name);
...
DumpTargets:
	target = all_targets;
	char*name_ext;
	command_print(CMD_CTX, "    TargetName         Type       Endian State       ");
	command_print(CMD_CTX, "--  ------------------ ---------- ------ ------------");
	while (target)
	{
		const char *state = target_state_name( target );
		char marker = ' ';
		...
		char name_ext[64];
		target_name_ext(target, name_ext);
		command_print(CMD_CTX, "%2d%c %-18s %-10s %-6s %-6s %s",
		              target->target_number, marker, name_ext, target_type_name(target),
...
COMMAND_HANDLER(handle_reg_command)
{
	...
	if(CMD_ARGC == 0) {
		...
		while(cache) {
			...
			command_print(CMD_CTX, "     Name    Type       Value            Code");
			//                      37   42      50        60                78
			command_print(CMD_CTX, "---  ------  --------  ----------------  --------------------");
			for(i = 0, reg = cache->reg_list; i < cache->num_regs; i++, reg++, count++) {
				/*if(reg->physical)*/ {
					if(target->state == TARGET_HALTED && *reg->valid == 0)
						reg->desc->type->get(reg);
					reg->desc->type->print(reg, content);
					command_print(CMD_CTX, "%3i  %s", count, content);
				}
				/*if(*reg->valid) {
					value = buf_to_str(reg->value, reg->size, 16);
					command_print(CMD_CTX, "(%i) %s (/%" PRIu32 "): 0x%s%s",
									count, reg->name, reg->size, value, *reg->dirty ? " (dirty)" : "");
					free(value);
				}
				else {
	...
	if ((CMD_ARGC == 1) || ((CMD_ARGC == 2) && !((CMD_ARGV[1][0] >= '0') && (CMD_ARGV[1][0] <= '9'))))
	{
		...
		reg->desc->type->print(reg, content);
		command_print(CMD_CTX, "Name    Type       Value            Code");
		command_print(CMD_CTX, "%s", content);
		return ERROR_OK;
	}
	...
	if (CMD_ARGC == 2)
	{
		uint8_t *buf = malloc(DIV_ROUND_UP(reg->desc->size, 8));
		str_to_buf(CMD_ARGV[1], strlen(CMD_ARGV[1]), buf, reg->desc->size, 0);

		if(target->state == TARGET_HALTED)
			reg->desc->type->set(reg, buf);
		else {
			command_print(CMD_CTX, "Traget not halted, can not modify registers");
			return ERROR_TARGET_NOT_HALTED;
		}
...
int target_wait_state(struct target *target, enum target_state state, int ms)
{
	...
	for (;;)
	{
		...
		if (once)
		{
			once = 0;
			then = timeval_ms();
			LOG_DEBUG("waiting for target %s...", Jim_Nvp_value2name_simple(nvp_target_state,state)->name);
		}
		...
		if ((cur-then) > ms)
		{
			LOG_ERROR("timed out while waiting for target %s", Jim_Nvp_value2name_simple(nvp_target_state,state)->name);
			return ERROR_FAIL;
		}
	}
...
COMMAND_HANDLER(handle_halt_command)
{
	...
	if (CMD_ARGC == 1)
	{
		unsigned wait_local; // millisecond
		retval = parse_uint(CMD_ARGV[0], &wait_local);
		if (ERROR_OK != retval)
			return ERROR_COMMAND_SYNTAX_ERROR;
		if (!wait_local) // true if timeout is zero, like "halt 0", will not wait for halt
			return ERROR_OK;
	}

	// wait for halt, call poll()
	// if timeout is not specified, default its value to 5000ms
...
static int target_read_memory_fast(struct target *target,
		uint32_t address, uint32_t size, uint32_t count, uint8_t *buffer)
{
	return target_read_buffer(target, address, size * count, buffer);
}
		...
		uint32_t *min_address, uint32_t *max_address)
{
	...
	if (CMD_ARGC == 5)
	{
		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[4], *max_address);// get max_length
...
static int handle_bp_command_list(struct command_context *cmd_ctx)
{
	...
	while (breakpoint)
	{
		...
		{
			// buf_len: bit as unit
			char* buf = buf_to_str(breakpoint->orig_instr, breakpoint->length * 8, 16);
			command_print(cmd_ctx, "sw bp ID: %d, Addr: 0x%X, Size: %d, Set: %i, Origin Instr: 0x%s",
		...
		{
			command_print(cmd_ctx, "hw bp ID: %d, Addr: 0x%X, Size: %d, Set: %i",
			              breakpoint->unique_id,
						  breakpoint->address,
...
COMMAND_HANDLER(handle_wp_command)
{
	...
	if (CMD_ARGC == 0)
	{
		...
		while (watchpoint)
		{
			command_print(CMD_CTX, "watchpoint %i, m/d/a: %i"
	...
	// set default value
  enum watchpoint_mode mode = MODE_ALL;
	uint32_t addr = 0;
	uint32_t length = 4; // 'length' defaults to 4 for mapu spu.
  enum watchpoint_rw type = WPT_ACCESS;
	uint32_t data_value = 0x0;
  uint32_t data_mask = 0xffffffff; // check all bits by default.

  // the first arg determines its monitoring mode
  switch(CMD_ARGV[0][0])
  {
    case 'm': // wp m addr length [type]
      mode = MODE_MEM;
      switch(CMD_ARGC)
      {
        case 4:
          switch(CMD_ARGV[3][0])
          {
            ...
            case 'a':
              ...
              break;
            default:
              LOG_ERROR("invalid watchpoint type ('%c') for monitoring memory address", CMD_ARGV[3][0]);
              return ERROR_COMMAND_SYNTAX_ERROR;
          }
        case 3 :
        ...
        default:
          command_print(CMD_CTX, "usage: wp [ 'm' address length [(r|w|a)] ]");
          return ERROR_COMMAND_SYNTAX_ERROR;
      }
      break;

    case 'd': // wp d value mask [type]
      mode = MODE_DATA;
      switch(CMD_ARGC)
      {
        case 4:
          switch(CMD_ARGV[3][0])
          {
            ...
            case 'a':
              ...
              break;
            default:
              LOG_ERROR("invalid watchpoint type ('%c') for monitoring data", CMD_ARGV[3][0]);
              return ERROR_COMMAND_SYNTAX_ERROR;
          }
        case 3:
          COMMAND_PARSE_NUMBER(u32, CMD_ARGV[2], data_mask);
        case 2:
          COMMAND_PARSE_NUMBER(u32, CMD_ARGV[1], data_value);
          break;

        default:
          command_print(CMD_CTX, "usage: wp [ 'd' value [ mask [(r|w|a)] ] ]");
          return ERROR_COMMAND_SYNTAX_ERROR;
      }
      break;
    ...
    case 'a':
      ...
      switch(CMD_ARGC)
      {
        case 6:
          switch(CMD_ARGV[5][0])
          {
            ...
            case 'a':
              ...
              break;
            default:
              LOG_ERROR("invalid watchpoint type ('%c')", CMD_ARGV[5][0]);
              return ERROR_COMMAND_SYNTAX_ERROR;
          }
        ...
        default:
          command_print(CMD_CTX, "usage: wp [ 'a' addr length value [ mask [(r|w|a)] ] ]");
          return ERROR_COMMAND_SYNTAX_ERROR;
      }
    ...
    default:
      LOG_ERROR("invalid watchpoint mode ('%c')", CMD_ARGV[0][0]);
      return ERROR_COMMAND_SYNTAX_ERROR;
  }

	int retval = watchpoint_add(target, mode, addr, length, type, data_value, data_mask);
	if (ERROR_OK != retval)
...
COMMAND_HANDLER(handle_finish_command) {
	LOG_DEBUG("-");

	struct target *target = get_current_target(CMD_CTX);
	return target_finish(target);
}
...
static int target_create(Jim_GetOptInfo *goi)
{
	...
	/* COMMAND */
	Jim_GetOpt_Obj(goi, &cmd_body);
	/* does this command exist? A yes means a target with the same name already exists.
	 * since we always register target name as a target-specific root command. */
	jim_cmd = Jim_GetCommand(goi->interp, cmd_body, JIM_ERRMSG);
	if (jim_cmd) {
		cp = Jim_GetString(cmd_body, NULL);
	...
	target->spu_mask = -1; // default value
	target->virt = false;
...
static int target_createv(Jim_GetOptInfo *goi)
{
	Jim_Obj *cmd_body;
	Jim_Cmd *jim_cmd;
	const char *cp;
	...
	if (goi->argc < 2) {
		Jim_WrongNumArgs(goi->interp, 1, goi->argv, "?name? ?type? ?mask? [..options...]");
		return JIM_ERR;
	}

	/* COMMAND, also the specified target name */
	Jim_GetOpt_Obj(goi, &cmd_body);
	/* does this command exist? A yes means a target with the same name already exists.
	 * since we always register target name as a target-specific root command. */
	jim_cmd = Jim_GetCommand(goi->interp, cmd_body, JIM_ERRMSG);
	if (jim_cmd) {
		cp = Jim_GetString(cmd_body, NULL);
		Jim_SetResultFormatted(goi->interp, "command/target: %s already exists", cp);
		return JIM_ERR;
	}
	...
	/* Target mask.
	 * A bit set to 1 means jtag commands will work upon the corresponding target. */
	e = Jim_GetOpt_String(goi, &cp2, NULL);
	unsigned long mask = strtoul(cp2, NULL, 0);    // mask will be used later.

	/* Create it */
	target = calloc(1,sizeof(struct target));
	target->spu_mask = mask;
	target->virt = true;
	/* set target number */
	target->target_number = new_target_number();
...
static int jim_target_createv(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	Jim_GetOptInfo goi;
	Jim_GetOpt_Setup(&goi, interp, argc - 1, argv + 1);
	if (goi.argc < 3)
...
static const struct command_registration target_subcommand_handlers[] = {
	...
	},
	{
			...
			/* REVISIT this should be COMMAND_CONFIG ... */
			.mode = COMMAND_ANY,
			.jim_handler = jim_target_createv,
			.usage = "name mask type '-chain-position' name [options ...]",
			.help = "Creates and selects a new vitual target",
		},
	{
		.name = "current",
		.mode = COMMAND_ANY,
...
static const struct command_registration target_exec_command_handlers[] = {
	...
	{
		.name = "finish",
		.handler = handle_finish_command,
		.mode = COMMAND_EXEC,
		.help = "finish target execution",
	},
	{
		.name = "resume",
	...
	},
	{
		...
		.usage =   "[m addr length [(a|w|r)]], or:\n"
		         "wp [d value [mask [(a|w|r)]]], or:\n"
		         "wp [a addr length value [mask [(a|w|r)]]]",
	},
	{
		.name = "rwp",
