...
struct gdb_service
{
   ...
   *  element 1: coreid to be displayed at next resume (1 till n).
   *             0 means resume all cores displayed  */
	int32_t core[2];
};
...
struct target
{
	...
	uint32_t spu_mask;	// a bit set to 1 means a jtag command will work upon the corresponding target.
	bool virt; // 1 if it's a virtual target, which is a group of targets.

	bool     gdb_connected;
	uint16_t gdb_port;  // each target has a gdb port.

	uint32_t cs_count;  // how many times we receives a "c"/"s" packet.
	                    // This is valuable for lldb to initialize a spu program.

	uint32_t pid;       // for lldb debug, each target is a remote process.
	uint32_t tid;       // a tid seems necessary for lldb debug
	bool process_exited;
...
// yangyy: I believe this struct is for SMP.
struct target_list {
	struct target *target;
