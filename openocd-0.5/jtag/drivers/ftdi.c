...
struct ftdi_context *ftdi_new(void) {
	struct ftdi_context * ftdi = (struct ftdi_context *) malloc(
...
int ftdi_usb_reset(struct ftdi_context *ftdi) {
	...
	if (usb_control_msg(ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
			SIO_RESET_REQUEST, SIO_RESET_SIO, ftdi->index, NULL, 0,
...
int ftdi_usb_purge_rx_buffer(struct ftdi_context *ftdi) {
	...
	if (usb_control_msg(ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
			SIO_RESET_REQUEST, SIO_RESET_PURGE_RX, ftdi->index, NULL, 0,
			ftdi->usb_write_timeout) != 0)
...
int ftdi_usb_purge_tx_buffer(struct ftdi_context *ftdi) {
	...
	if (usb_control_msg(ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
			SIO_RESET_REQUEST, SIO_RESET_PURGE_TX, ftdi->index, NULL, 0,
			ftdi->usb_write_timeout) != 0)
				...
				for (i = 1; i < num_of_chunks; i++)
					memmove(
							ftdi->readbuffer + ftdi->readbuffer_offset
									+ (packet_size - 2) * i,
							ftdi->readbuffer + ftdi->readbuffer_offset
									+ packet_size * i, packet_size - 2);
				if (chunk_remains > 2) {
					memmove(
							ftdi->readbuffer + ftdi->readbuffer_offset
									+ (packet_size - 2) * i,
							ftdi->readbuffer + ftdi->readbuffer_offset
									+ packet_size * i, chunk_remains - 2);
					ret -= 2 * num_of_chunks;
				} else
...
int ftdi_read_eeprom(struct ftdi_context *ftdi, unsigned char *eeprom) {
	...
	for (i = 0; i < ftdi->eeprom_size / 2; i++) {
		if (usb_control_msg(ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE,
				SIO_READ_EEPROM_REQUEST, 0, i, eeprom + (i * 2), 2,
				ftdi->usb_read_timeout) != 2)
			ftdi_error_return(-1, "reading eeprom failed");
	}
...
int ftdi_erase_eeprom(struct ftdi_context *ftdi) {
	...
	if (usb_control_msg(ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
			SIO_ERASE_EEPROM_REQUEST, 0, 0, NULL, 0, ftdi->usb_write_timeout)
			!= 0)
