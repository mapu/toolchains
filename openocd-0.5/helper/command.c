		...
		unsigned argc, Jim_Obj *const *argv)
{
	...
	char * dbg = alloc_printf("user cmd: %s, internal cmd:", name);
		...
		struct command *parent, const struct command_registration *cr)
{
	...
	else if (NULL != c->handler || NULL != c->parent)
...
// original: struct command_context* command_init(const char *startup_tcl, Jim_Interp *interp)
struct command_context* command_init(const char *filename, Jim_Interp *interp)
{
	...
	// original: if (Jim_Eval_Named(interp, startup_tcl, "embedded:startup.tcl",1) == JIM_ERR)
	if (Jim_EvalFile(interp, filename) == JIM_ERR)
	{
