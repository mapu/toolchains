...
int parse_config_file(struct command_context *cmd_ctx)
{
	...
	char *script_path = calloc(1,128);
	char *script_cmd  = calloc(1,128);

	readlink("/proc/self/exe", script_path, 128);
	dirname(script_path);

#ifdef SIM
  strcat(script_path, "/openocd-sim.cfg");
#else
  strcat(script_path, "/openocd.cfg");
#endif

	sprintf(script_cmd, "script %s", script_path);

	if (!config_file_names) {
		command_run_line(cmd_ctx, script_cmd);
		return ERROR_OK;
	}
