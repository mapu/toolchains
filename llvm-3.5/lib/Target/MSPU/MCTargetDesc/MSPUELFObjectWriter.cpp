...
unsigned MSPUELFObjectWriter::GetRelocType(const MCValue &Target,
                                           const MCFixup &Fixup,
                                           bool IsPCRel) const {
...
// Fix
void MSPUELFObjectWriter::adjustFixupOffset(const MCFixup &Fixup,
...
const MCSymbol *
MSPUELFObjectWriter::ExplicitRelSym(const MCAssembler &Asm,
                                    const MCValue &Target,
                                    const MCFragment &F,
                                    const MCFixup &Fixup,
                                    bool IsPCRel) const {
  return &Target.getSymA()->getSymbol();
}

MCObjectWriter *
llvm::createMSPUELFObjectWriter(raw_ostream &OS, uint8_t OSABI) {
