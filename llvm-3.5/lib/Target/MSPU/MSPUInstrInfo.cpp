...
MachineInstr *
MSPUInstrInfo::emitFrameIndexDebugValue(MachineFunction &MF,
											int FrameIx,
											uint64_t Offset,
											const MDNode *MDPtr,
											DebugLoc dl) const
{
	MachineInstrBuilder MIB = BuildMI(MF, dl, get(MSPUInst::DBG_VALUE))
...
unsigned
MSPUInstrInfo::getGlobalBaseReg(MachineFunction *MF) const
									{
	MSPUMachineFunctionInfo *MSPUFI = MF->getInfo<MSPUMachineFunctionInfo>();
	unsigned GlobalBaseReg = MSPUFI->getGlobalBaseReg();
	if(GlobalBaseReg != 0)
...
//	MachineBasicBlock::iterator MBBI = FirstMBB.begin();
	...
	GlobalBaseReg = RegInfo.createVirtualRegister(&MSPUReg::I32RegRegClass);

	DebugLoc dl;

//	BuildMI(FirstMBB, MBBI, dl, get(MSPUInst::GETPCX), GlobalBaseReg);
	MSPUFI->setGlobalBaseReg(GlobalBaseReg);
	return GlobalBaseReg;
}
