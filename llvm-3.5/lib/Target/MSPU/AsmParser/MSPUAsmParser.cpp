...
public:
  MSPUAsmParser(MCSubtargetInfo &sti, MCAsmParser &par, const MCInstrInfo &MII,
...
static int mspuinstlex(YYSTYPE * yylval, MSPU::InstLineToParse &InstLine) {
  ...
  case _Imm: {
    ...
    while (mcparser->getTok().getLoc().getPointer() <= loc.getPointer())
      mcparser->Lex();
...
bool MSPU::MSPUAsmParser::mnemonicIsValid(StringRef ref, unsigned u) {
  YYSTYPE yylval;
  const char* start = ref.data();
...
bool MSPU::MSPUAsmParser::ParseInstruction(ParseInstructionInfo &Info,
  ...
  if (*lineEnd == ';' && *(lineEnd + 1) == ';') lineEnd += 2;
  ...
  if (lineStart <= lineEnd) {
    /*mclexer.setCurPtr(lineStart);*/
    while (mcparser->getTok().getLoc().getPointer() < lineStart)
      mcparser->Lex();
