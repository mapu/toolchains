...
#ifdef ARCH_MAPU
        // MaPU hacking
        create |= (arch->GetTriple().getVendor() == llvm::Triple::UnknownVendor) &&
...
PlatformRemoteGDBServer::GetSupportedArchitectureAtIndex (uint32_t idx, ArchSpec &arch)
{
  ...
  {
    case 0:
      arch.SetArchitecture(eArchTypeELF, llvm::ELF::EM_MSPU, LLDB_INVALID_CPUTYPE);
      return arch.IsValid();
...
PlatformRemoteGDBServer::GetSoftwareBreakpointTrapOpcode (Target &target, BreakpointSite *bp_site)
{
  ...
  if(arch.GetMachine() == llvm::Triple::mspu)
    return 4;
  // end MaPU
#endif
    return 0;
