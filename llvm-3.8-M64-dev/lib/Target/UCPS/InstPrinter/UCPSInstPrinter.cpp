...
void UCPSInstPrinter::printInst(const MCInst *MI, raw_ostream &O,
	...
	do {
	 ...
	 /*yangl if (curinst->getOpcode() != UCPS::NOP) {
	    O << split;
...
void UCPSInstPrinter::printFlags(const MCInst *MI, unsigned OpNo,
                                     raw_ostream &O) const {
	const MCOperand& MO = MI->getOperand(OpNo);
...
void UCPSInstPrinter::printKIReg(const MCInst *MI, unsigned OpNo,
                                     raw_ostream &O) const {
	const MCOperand& MO = MI->getOperand(OpNo);
...
void UCPSInstPrinter::printIPath(const MCInst *MI, unsigned OpNo,
                                     raw_ostream &O) const {
	const MCOperand& MO = MI->getOperand(OpNo);
...
void UCPSInstPrinter::printMACCTI(const MCInst *MI,
                                      unsigned int OpNo,
                                      raw_ostream &O) const {
...
void UCPSInstPrinter::printSHUT(const MCInst *MI,
                                    unsigned int OpNo,
                                    raw_ostream &O) const {
	printOperand(MI, OpNo, O);
  O << ".";
