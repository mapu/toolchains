...
///
  MMPULiteDisassembler(const MCSubtargetInfo &STI,
    ...
    : MCDisassembler(STI), Scheduler(new MMPULiteScheduler()), RegInfo(Info) {}
...
static DecodeStatus readInsnLine(const MemoryObject &region,
  ...
  if (region.readBytes(address, numBytes, line.getCodeBuf()) == -1) {
		numBytes = 0;
...
static bool getReg(const void *D, unsigned RC, unsigned& RegNo) {
	...
	unsigned RegPtr;
	for (unsigned i = 0;
       i < Dis->getRegInfo()->getRegClass(RC).getNumRegs();
       i++) {
		RegPtr = Dis->getRegInfo()->getRegClass(RC).getRegister(i);
		if (RegNo == Dis->getRegInfo()->getEncodingValue(RegPtr)) {
      RegNo = RegPtr;
      return true;
    }
	}
...
getInstruction(MCInst &Inst, uint64_t &Size, const MemoryObject &Region,
  ...
  if (!result) return result;
	...
	Scheduler->Schedule(Address, &Inst, Sym);
...
static MCDisassembler *createMMPULiteDisassembler(const Target &T,
  ...
  return new MMPULiteDisassembler(STI, T.createMCRegInfo(""), true);
}
