...
private:
  ...
  void RenameHMacro(SmallVectorImpl<MCParsedAsmOperand*> &_Operands);
public:
  ...
  bool HandleHMacroEntry(StringRef Name, SMLoc NameLoc, const HMacro *M, StringRef Prefix);
...
bool MMPULite::MMPULiteAsmParser::MatchAndEmitInstruction(SMLoc IDLoc,
                            unsigned &Opcode,
                            SmallVectorImpl<MCParsedAsmOperand*> &Operands,
                            MCStreamer &Out, unsigned &ErrorInfo,
                            bool MatchingInlineAsm) {
  ...
  do {
    for(unsigned i=0; ops && i < ops->size(); i++) {
      op = static_cast<MMPULiteAsmOperand *> ((*ops)[i]);
      switch(op->getKind()) {
      default:
        return false;
...
int MMPULite::MMPULiteAsmParser::AnalyzeHMacro(unsigned &Sym) {
  MCLoopBlock* MB;
  MCBlockTy PreType = Lite::None, CurType;
  int64_t MaxCnt = 0;
  ...
  for (unsigned i = 0; i < ActiveHMacros.size(); i++) {
    if (ActiveHMacros[i]->getContainer(VAddr, MB)) {
      ...
      if ((CurType == Sequential && PreType == Repeat) ||
        ...
        else if (PreType == Lite::None)
          MaxCnt = MB->getCount();
        else // loop with repeat is illegal
          return -1;
      }
      if (CurType != Lite::None) PreType = CurType;
    }
  ...
  for (unsigned i = 0; i < ActiveHMacros.size(); i++) {
    if (ActiveHMacros[i]->getContainer(VAddr + 1, MB) || 
      ...
      if (MB->getType(VAddr + 1) != Lite::None &&
...
void MMPULite::MMPULiteAsmParser::RenameHMacro(MCParsedInst *PI) {
  for(unsigned i= 0; i < PI->size(); ++i) {
    MMPULite::MMPULiteAsmOperand *op =
      static_cast<MMPULite::MMPULiteAsmOperand*> (PI->getOperand(i));
    if (op->isHMacro()) {
...
RenameHMacro(SmallVectorImpl<MCParsedAsmOperand*> &_Operands) {
  for(unsigned i= 0; i < _Operands.size(); ++i) {
    MMPULite::MMPULiteAsmOperand *op =
      static_cast<MMPULite::MMPULiteAsmOperand*> (_Operands[i]);
    if (op->isHMacro()) {
      const HMacro *HM = HMacroMap.lookup(StringRef(*(op->getHMacro())));
      std::string OldName = *(op->getHMacro());
      op->getHMacro()->append(Twine("\1").str());
