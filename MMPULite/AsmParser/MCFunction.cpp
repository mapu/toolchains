...
bool MCParsedInst::hasRepeat(unsigned &OpNo) const {
  for(OpNo = 0; OpNo < Operands.size(); OpNo++) {
    MMPULite::MMPULiteAsmOperand *op =
      static_cast<MMPULite::MMPULiteAsmOperand*>(Operands[OpNo]);
    if (op->isToken() && 
...
bool MCParsedInst::hasEmbeddedHMacro(unsigned &OpNo) const {
  for(OpNo = 0; OpNo < Operands.size(); OpNo++) {
    ...
    if (op->isHMacro() && (OpNo + 1 < Operands.size())) {
      op = static_cast<MMPULite::MMPULiteAsmOperand*>(Operands[OpNo + 1]);
...
void MCLoopBlock::ExpandEmbeddedHM(StringMap<HMacro*> &Map) {
  ...
  while (!EmbeddedHMs.empty()) {
    ...
    errs() << "R:" << EmbeddedHMs.front()->blocksize() << "|" << start
           << "L:" << LHS.size() << "|" << Start << "\n";
    if (start > Start) {
      SmallVector<MCParsedAsmOperand *, 8> Nops;
      Nops.push_back(MMPULiteAsmOperand::createOpc(NOP));
      if (start - Start == 1) {
...
void MCFunction::addParsedInst(MCParsedInst *Inst) {
  ...
  if (cachedLabel) {
    ...
    while (PreviousInst && PreviousInst->hasLoop(OpNo)) {
      op = static_cast<MMPULite::MMPULiteAsmOperand*> (PreviousInst->getOperand(OpNo + 1));
      ...
      if (op->isExpr()) {
        ...
        if (CE) return; //ignored
          ...
          if (SE) {
            op = static_cast<MMPULite::MMPULiteAsmOperand*> (PreviousInst->getOperand(OpNo + 2));
            PendingLabels.push_back(&(SE->getSymbol()));
            ki = op->getImm();
            op = static_cast<MMPULite::MMPULiteAsmOperand*> (PreviousInst->getOperand(OpNo + 3));
            count = -op->getImm();
          }
        }
      }
      ...
      while (PreviousInst->hasEmbeddedHMacro(OpNoH)) {
        if (OpNoH > OpNo) break;
        Embedded = true;
        op = static_cast<MMPULite::MMPULiteAsmOperand*>(PreviousInst->getOperand(OpNoH));
  ...
  if (Inst->hasRepeat(OpNo)) {
    op = static_cast<MMPULite::MMPULiteAsmOperand*>(Inst->getOperand(OpNo + 1));
    count = op->getImm();
    op = static_cast<MMPULite::MMPULiteAsmOperand*>(Inst->getOperand(OpNo));
    if (op->getOpc() == MMPULite::REPEATIMM && count == 0) count = 1024;
    else if (op->getOpc() == MMPULite::REPEATK) {
      ki = count;
      op = static_cast<MMPULite::MMPULiteAsmOperand*>(Inst->getOperand(OpNo + 2));
      count = -op->getImm();
    }
    if (count != 1) {
      if (CurBlock->size() != 0) {
    ...
    if (op->isExpr()) {
      ...
      if (CE) return; //ignored
        ...
        const MCSymbolRefExpr *SE = dyn_cast<MCSymbolRefExpr>(op->getExpr());
        if (SE) {
          op = static_cast<MMPULite::MMPULiteAsmOperand*>
  ...
  if (Inst->hasRepeat(OpNo)) {
    ...
    while (Inst->hasEmbeddedHMacro(OpNoH)) {
      if (OpNoH > OpNo) break;
      Embedded = true;
      op = static_cast<MMPULite::MMPULiteAsmOperand*>(Inst->getOperand(OpNoH));
      CurBlock->addEmbeddedHM(new MCFunction(*op->getHMacro(), Index - 1, Ctx));
      Inst->delOperand(OpNoH);
      Inst->delOperand(OpNoH);
    }
