...
public:
  ...
  Target_mmpu()  : Sized_target<size, big_endian>(&mmpu_info)  { }
...
//////////////////////////////////////////////////////////////////////
template<>
Target::Target_info Target_mmpu<32, false>::mmpu_info =
{
  32,			// size
  false,		// is_big_endian
...
public:
  Target_selector_mmpu()
    : Target_selector(elfcpp::EM_NONE, size, big_endian,
					  "elf32-mmpu", "elf32mmpu")  { }

  Target* do_recognize(int machine, int, int){
	if( machine == elfcpp::EM_MMPU) 	return this->instantiate_target();
	else 							return NULL;
  }
...
template<int size, bool big_endian>
void
Target_mmpu<size, big_endian>::gc_process_relocs(
    Symbol_table*                         symtab,
    Layout*                               layout,
    Sized_relobj_file<size, big_endian>*  object,
    unsigned int                          data_shndx,
  ...
  gold::gc_process_relocs<size, big_endian, Mmpu, elfcpp::SHT_RELA, Scan,
			  typename Target_mmpu::Relocatable_size_for_reloc>(
    symtab,
    layout,
    this,
    object,
...
template<int size, bool big_endian>
void
Target_mmpu<size, big_endian>::scan_relocs(
    Symbol_table*         symtab,
    Layout*               layout,
    Sized_relobj_file<size, big_endian>* object,
    unsigned int          data_shndx,
  ...
  gold::scan_relocs<size, big_endian, Mmpu, elfcpp::SHT_RELA, Scan>(
    symtab,
    layout,
    this,
    object,
...
/****************************************************************/
template<int size, bool big_endian>
void Target_mmpu<size, big_endian>::relocate_for_relocatable(
  const Relocate_info<size, big_endian>*      relinfo,
  unsigned int                                sh_type,
  const unsigned char*                        prelocs,
  size_t                                      reloc_count,
...
template<int size, bool big_endian>
inline void
Target_mmpu<size, big_endian>::Scan::local(
  Symbol_table*                         symtab, 
  Layout*                               layout, 
  Target_mmpu*                           target,
  Sized_relobj_file<size, big_endian>*  object,
...
template<int size, bool big_endian>
inline void
Target_mmpu<size, big_endian>::Scan::global(
  Symbol_table*                         symtab, 
  Layout*                               layout, 
  Target_mmpu*                           target,
  Sized_relobj_file<size, big_endian>*  object,
...
Target_mmpu<size, big_endian>::Relocate::relocate(
  const Relocate_info<size, big_endian>*      relinfo,
  Target_mmpu*                             	  target,
  Output_section*                                   ,
  size_t                                      relnum,
