                                ...
                                SDLoc DL, SelectionDAG &DAG) const
{
  ...
  return DAG.getNode(MSPU_ISD::RET, DL, MVT::Other, &RetOps[0], RetOps.size());
}
                                  ...
                                  SmallVectorImpl<SDValue> &InVals) const
{
  ...
  if (!MemArgChains.empty())
    Chain = DAG.getNode(ISD::TokenFactor, DL, MVT::Other,
                        &MemArgChains[0], MemArgChains.size());
  ...
  Chain = DAG.getNode(MSPU_ISD::CALL, DL, VTList, &Ops[0], Ops.size());
  Glue = Chain.getValue(1);
...
/// combiner.
void
MSPUTargetLowering::computeMaskedBitsForTargetNode(const SDValue Op,
													APInt &KnownZero,
													APInt &KnownOne,
    ...
    switch(extTy) {
      ...
      case ISD::EXTLOAD: {
        ...
        return DAG.getExtLoad(ISD::ZEXTLOAD, dl, MVT::i32, node->getOperand(0),
                              ...
                              node->isNonTemporal(),node->getAlignment(), NULL);
      }
...
LowerBR_CC(SDValue Op, SelectionDAG &DAG)
{
	...
	if(LHS.getValueType() == MVT::i32) {
		...
		CompareFlag = DAG.getNode(MSPU_ISD::CMPICC, dl, VTs, Ops, 2).getValue(1);
		if(MSPU_CC == ~0U)
