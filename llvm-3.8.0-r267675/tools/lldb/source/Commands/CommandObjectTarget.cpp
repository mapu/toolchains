...
#ifdef ARCH_MAPU
#pragma mark CommandObjectTargetDownload

//-------------------------------------------------------------------------
// target download
//-------------------------------------------------------------------------

class CommandObjectTargetDownload : public CommandObjectParsed
{
public:
    CommandObjectTargetDownload (CommandInterpreter &interpreter) :
        CommandObjectParsed (interpreter,
                             "target download" /* name */,
                             "Download current target into remote debug platform.",
                             "target download" /* syntax */,
                             0)
    {
    }

    virtual
    ~CommandObjectTargetDownload ()
    {
    }
...
protected:
    ...
    DoExecute (Args& args, CommandReturnObject &result)
    {
        ...
        if(!target)
        {
            result.AppendError ("No active target.\n");
            result.SetStatus (eReturnStatusFailed);
        }

        const ArchSpec & arch = target->GetArchitecture ();
        if(arch.GetCore() != ArchSpec::eCore_mspu)
        {
            result.AppendError("*target download* is only valid for MaPU SPU.\n");
            result.SetStatus(eReturnStatusFailed);
        }
        ...
        if(!process)
        {
             result.AppendError ("No active process for current target.\n");
             result.SetStatus (eReturnStatusFailed);
        }

        // we only need the *main* module
        SectionLoadList& loadlist = target->GetSectionLoadList();
        Module* exec = target->GetExecutableModulePointer ();

        ObjectFile* obj = exec->GetObjectFile();
        SectionList * seclist = obj->GetSectionList ();

        for(size_t id=0; id < seclist->GetNumSections(0); id++)
        {
            const lldb::SectionSP sec = seclist->GetSectionAtIndex (id);
            lldb::addr_t allsz = sec->GetByteSize (); // byte size to be filled into memory
            if(allsz > 0)
            {
                addr_t addr = loadlist.GetSectionLoadAddress(sec);
                DataExtractor data;
                size_t sz = obj->ReadSectionData (sec.get(), data);
                Error error;
                lldb::offset_t pos = 0;
                const void* buf = data.GetData (&pos, sz);
                process->WriteMemory(addr, buf, sz, error);

                if(sz<allsz)
                {
                    int len = allsz-sz;
                    char* null = new char[len];
                    ::memset(null, 0, len);
                    process->WriteMemory(addr+sz, null, len, error);
                    delete[] null;
                }
            }
        }
...
#ifdef ARCH_MAPU
    LoadSubCommand ("download",  CommandObjectSP (new CommandObjectTargetDownload (interpreter)));
#endif
}
