//===- MSPUMachineScheduler.cpp - MI Scheduler for MSPU -------------===//
//
...
void MSPUVLIWScheduleMI::postprocessDAG() {
...
/// empirically.
bool MSPUResourceModel::isResourceAvailable(SUnit *SU) {
  if (!SU || !SU->getInstr())
    return false;
...
bool MSPUResourceModel::reserveResources(SUnit *SU) {
...
void MSPUVLIWScheduleMI::schedule() {
  DEBUG(dbgs()
...
void MSPUConvergingSched::initialize(ScheduleDAGMI *dag) {
  DAG = static_cast<MSPUVLIWScheduleMI*>(dag);
  ...
  Top.ResourceModel = new MSPUResourceModel(STI, DAG->getSchedModel());
  Bot.ResourceModel = new MSPUResourceModel(STI, DAG->getSchedModel());

  assert((!llvm::ForceTopDown || !llvm::ForceBottomUp) &&
         "-misched-topdown incompatible with -misched-bottomup");
}

void MSPUConvergingSched::releaseTopNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
void MSPUConvergingSched::releaseBottomNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
bool MSPUConvergingSched::SchedBoundary::checkHazard(SUnit *SU) {
...
void MSPUConvergingSched::SchedBoundary::releaseNode(SUnit *SU,
                                                     unsigned ReadyCycle) {
...
void MSPUConvergingSched::SchedBoundary::bumpCycle() {
...
void MSPUConvergingSched::SchedBoundary::bumpNode(SUnit *SU) {
  bool startNewCycle = false;
...
void MSPUConvergingSched::SchedBoundary::releasePending() {
...
void MSPUConvergingSched::SchedBoundary::removeReady(SUnit *SU) {
...
SUnit *MSPUConvergingSched::SchedBoundary::pickOnlyChoice() {
  if (CheckPending)
    releasePending();
...
#ifndef NDEBUG
void MSPUConvergingSched::traceCandidate(const char *Label,
                                             const ReadyQueue &Q,
...
int MSPUConvergingSched::SchedulingCost(ReadyQueue &Q, SUnit *SU,
...
MSPUConvergingSched::CandResult MSPUConvergingSched::
...
SUnit *MSPUConvergingSched::pickNodeBidrectional(bool &IsTopNode) {
...
SUnit *MSPUConvergingSched::pickNode(bool &IsTopNode) {
...
/// does.
void MSPUConvergingSched::schedNode(SUnit *SU, bool IsTopNode) {
  if (IsTopNode) {
