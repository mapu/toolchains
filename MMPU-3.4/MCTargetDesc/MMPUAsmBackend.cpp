//===-- MMPUASMBackend.cpp - MMPU Asm Backend  ----------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the MMPUAsmBackend and MMPUELFObjectWriter classes.
//
//===----------------------------------------------------------------------===//
//
...
namespace {
	class MMPUAsmBackend: public MCAsmBackend {
			uint8_t OSABI;
		public:
			MMPUAsmBackend(const Target &T, uint8_t osabi)
			...
			{
				return createMMPUELFObjectWriter(OS, OSABI);
			}
			...
			{
				return MMPU::NumTargetFixupKinds;
			}

			const MCFixupKindInfo &getFixupKindInfo(MCFixupKind Kind) const
			{
				const static MCFixupKindInfo Infos[MMPU::NumTargetFixupKinds] = {
						// This table *must* be in the same order with those fixup_*_* kinds
            // defined in MMPUFixupKinds.h
            // relocation at last slot starts from byte 38 at bit 1+8
						// name    	          bit-offset  bits flags
						{ "fixup_MMPU_11", /* 17-8 */9,   11,   0},
				};
			...
			bool writeNopData(uint64_t Count, MCObjectWriter *OW) const
			{
				...
				if (Count % MMPU::SumEncodingBytes) return false;
				//uint64_t NumNops = Count / 4;
				for (uint64_t i = 0; i != Count; ++i)
					OW->Write8(0);
				return true;
			}
	}; // class MMPUAsmBackend

} // namespace

MCAsmBackend * llvm::createMMPUAsmBackend(const Target &T,
                                     const MCRegisterInfo &MRI,
                                     StringRef TT, StringRef CPU)
{
	uint8_t OSABI = MCELFObjectTargetWriter::getOSABI(Triple(TT).getOS());
	return new MMPUAsmBackend(T, OSABI);
}
