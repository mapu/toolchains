...
static void
ncx_slab_free_pages(ncx_slab_pool_t *pool, ncx_slab_page_t *page, ncx_uint_t pages);
static bool
...
static void *ncx_slab_alloc_locked(ncx_slab_pool_t *pool, size_t size);
static bool ncx_slab_free_locked(ncx_slab_pool_t *pool, void *p);
...
void ncx_slab_init(ncx_slab_pool_t *pool) {
  ...
  for (n = ncx_pagesize, ncx_pagesize_shift = 0; n >>= 1; ncx_pagesize_shift++) { /* void */
  }
  ...
  for (i = 0; i < n; i++) {
    slots[i].slab = 0;
    slots[i].next = &slots[i];   // 初始时每个大小的slot都是空的
    slots[i].prev = 0;
  }
  ...
  pool->pages->next = &pool->free;    // 这是最后一个free块, 指向头结点
   ...
   */
  pool->start = (u_char *) ncx_align_ptr((uintptr_t ) p + pages * sizeof(ncx_slab_page_t), ncx_pagesize);
...
ncx_slab_alloc_locked(ncx_slab_pool_t *pool, size_t size) {
  ...
  ncx_uint_t i, slot, shift, map_size;
  ...
  if (size >= ncx_slab_max_size) {
    debug("slab alloc: %lu", size);

    page = ncx_slab_alloc_pages(pool, (size - 1 + ncx_pagesize) >> ncx_pagesize_shift);
   ...
   * slab存在空闲obj
   */
  if (page->next != page) {  // 半满的才被slab管理, 不空的slab页肯定是可以用的
    /*
     * slab规模分为三种:
    ...
    if (shift < ncx_slab_exact_shift) {
       ...
       */
      map_size = (1 << (ncx_pagesize_shift - shift)) / (sizeof(uintptr_t) * 8);
      ...
      for (n = 0; n < map_size; n++) {
        ...
        if (bitmap[n] != NCX_SLAB_BUSY) {
          ...
          for (m = 1, i = 0; m; m <<= 1, i++) {
            ...
            /*
             * 接下来的逻辑是遍历其余整个bitmap
             * 如果分配了该obj后, 该page没有空闲obj，则把该page的管理节点从链表里删除，即该page处于“悬空”状态
             */
            if (bitmap[n] == NCX_SLAB_BUSY) {
              for (n = n + 1; n < map_size; n++) {
                if (bitmap[n] != NCX_SLAB_BUSY) {
              ...
              prev->next = page->next; // ncx_slab_page_t * 肯定是4字节对齐的. prev的低两位用来保存规模
              page->next->prev = page->prev;

              /*
               * NCX_SLAB_SMALL 表示 < ncx_slab_exact_shift 的slab类型
               */
              page->next = NULL;             // 清空当前满的page
              page->prev = NCX_SLAB_SMALL;   // 这个也是清空
            }
      ...
      } // 若page空闲, 肯定在上面退出了. 而page在slots里, 没有空闲是不可能的
      ...
      if (page->slab != NCX_SLAB_BUSY) {
        ...
        for (m = 1, i = 0; m; m <<= 1, i++) {
          ...
          if (page->slab == NCX_SLAB_BUSY) {  // 不需要循环了, 直接一个if就判断整个page是否已经满了
            prev = (ncx_slab_page_t *) (page->prev & ~NCX_SLAB_PAGE_MASK);
            prev->next = page->next;
            page->next->prev = page->prev;
       ...
       * 接下来的操作与之前类似
       */
      for (m = (uintptr_t) 1 << NCX_SLAB_MAP_SHIFT, i = 0; m & mask; m <<= 1, i++) {
        if ((page->slab & m)) {
          continue;
        }
...
bool ncx_slab_free(ncx_slab_pool_t *pool, void *p) {
  ...
  ret = ncx_slab_free_locked(pool, p);
...
static bool ncx_slab_free_locked(ncx_slab_pool_t *pool, void *p) {
  ...
  ncx_uint_t n, type, slot, shift, map_size;
  ...
  case NCX_SLAB_SMALL:
    ...
    m = (uintptr_t) 1 << (n & (sizeof(uintptr_t) * 8 - 1));           // 第m位
    n /= (sizeof(uintptr_t) * 8);                             // 第n个bitmap元素
    ...
    if (bitmap[n] & m) {
       ...
       */
      if (bitmap[0] & ~(((uintptr_t) 1 << n) - 1))   // TO-DO 看是否有地方动0号
      {
        goto done;
      }
  ...
  case NCX_SLAB_BIG:
     ...
     */
    m = (uintptr_t) 1 << ((((uintptr_t) p & (ncx_pagesize - 1)) >> shift) + NCX_SLAB_MAP_SHIFT);
...
ncx_slab_alloc_pages(ncx_slab_pool_t *pool, ncx_uint_t pages) {
  ...
  for (page = pool->free.next; page != &pool->free; page = page->next) {
     ...
     */
    if (page->slab >= pages)    // TO-DO 这个分配方式还是傻了. 不进行挑选, 只分配第一个满足条件的块, 可能造成更多的碎片
    {
      /*
...
#ifdef PAGE_MERGE
  if (pool->pages != page)      // 不在最前头, 看前头有没有free的
  {
    prev = page - 1;
...
void ncx_slab_stat(ncx_slab_pool_t *pool, ncx_slab_stat_t *stat) {
  ...
  for (i = 0; i < stat->pages; i++) {
    ...
    case NCX_SLAB_SMALL:
      ...
      obj_size = 1 << slab;
      map = (1 << (ncx_pagesize_shift - slab)) / (sizeof(uintptr_t) * 8);
  ...
  stat->used_pct = stat->used_size * 100.0 / stat->pool_size;

  info("pool_size : %lu bytes", stat->pool_size);
  info("used_size : %lu bytes", stat->used_size);
  info("used_pct  : %.2lf%%\n", stat->used_pct);

  info("total page count : %lu", stat->pages);
  info("free page count  : %lu\n", stat->free_page);

  info("small slab use page : %lu,\tbytes : %lu", stat->p_small, stat->b_small);
  info("exact slab use page : %lu,\tbytes : %lu", stat->p_exact, stat->b_exact);
  info("big   slab use page : %lu,\tbytes : %lu", stat->p_big, stat->b_big);
  info("page slab use page  : %lu,\tbytes : %lu\n", stat->p_page, stat->b_page);

  info("max free pages : %lu\n", stat->max_free_pages);
}
...
static bool ncx_slab_empty(ncx_slab_pool_t *pool, ncx_slab_page_t *page) {
  ...
  if (page->slab == 0)  // free的页, 而且前面还有free的页
  {
    return true;
  }
  ...
  if (page->next == NULL)    // 全满的页
  {
    return false;
  }

  // 以下为在slots链表内或者free链表内的页, 可以用next指针了

  next = page->next;

  while (next >= pool->pages)  // 找到头结点
  {
    next = next->next;
  }

  if (next == &pool->free)  // 是free的
  {
    return true;
  }
