  ...
  * )
  ...
  "h" )
    ...
    then
      ...
      echo "  test|-t|--test: run simulator to test the MaPU project."
      echo "  debug|-d|--debug: run simulator and gdb to debug the MaPU project."
      ...
      ;;
      "build" | "-b" | "--build" )
        echo "Usage: build|-b|--build SYSTEM_TYPE"
        echo -e "  SYSTEM_TYPE:\n\t--sim:\tBuild project for MaPU simulator;\n\t--chip:\tBuild project for MaPU chip."
      ;;
      "stat" | "-s" | "--status" )
        echo "Usage: stat|-s|--status"
      ;;
      "test" | "-t" | "--test" )
        echo "Usage: test|-t|--test"
      ;;
      "debug" | "-d" | "--debug" )
        echo "Usage: debug|-d|--debug"
      ;;
      "mgr" | "-m" | "--manage" )
  ...
  "n" )
    ...
    then
      ...
      echo "CONFIG_$projname=y"						>>$arch/autoconf.mk
      echo "APPSUBDIRS-\$(CONFIG_$projname)+=$projname"			>>$arch/autoconf.mk
    else
      mkdir $arch/$projname
      :>$arch/$projname/Makefile
  ...
  "r" )
    ...
    then
      ...
      if [ $arch = "arm" ]
      then
        sed -i'~' -e "s/CONFIG_$projname=.*$//g" -e "s/APPSUBDIRS-\$(CONFIG_$projname)+=.*$//g" $arch/autoconf.mk
      else
  ...
  "m_d" )
    ...
    then
      ...
      if [ "arch" = "arm" ]
      then
        sed -i'~' -e "s/CONFIG_$proj=y/CONFIG_$proj=n/" $arch/autoconf.mk
      else
  ...
  "m_e" )
    ...
    then
      ...
      if [ "arch" = "arm" ]
      then
        sed -i'~' -e "s/CONFIG_$proj=n/CONFIG_$proj=y/" $arch/autoconf.mk
      else
  ...
  "t" )
    ...
    ${MAPU_HOME}/simulator/mapu_sim.sh
  ;;
  "d" )
    echo "Debugging ..."
    ${MAPU_HOME}/simulator/mapu_sim.sh debug
  ;;
  esac
done
cd $working_path
