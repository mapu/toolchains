...
.text
    .global Initial
    .global FFTSF2048Ep0Test 
    .global FFTSF2048Ep1Test 
    .global FFTSF2048Ep2Test 
    .global FFTSF2048Ep3Test 
...
////////////////////////////////////////////////////////////////////////////////
//The unbanlanced part for BIURead.
//The main diffenence for BIURead in unbanlanced FFT is that the interval between
//the two pipe group is reduced duo to the reduction of stages in each epoch.
//  23      : the basic computation cycle of a float butterfly
//   number : the stages in each epoch.
//  20      : the pipe group number

.macro BIURead_un name number 
.hmacro BIURead_un_biu2_read_\name 
LPTO (1f ) @ (KI12); 
 BIU2.DM (A++,K++ )->M[I++] || Repeat @ (PipeGroup); 
Repeat @ ( (23 * (\number)) - PipeGroup ); 
1:
 BIU2.DM (A++,K++ )->M[I++] || Repeat @ (N); 
Repeat @ ( (23 * (\number)) - N ); 
.endhmacro
.endmacro
...
////////////////////////////////////////////////////////////////////////////////
//The unbanlanced part for BIU1.
//The main diffenence for BIURead in unbanlanced FFT is that the interval between
//the two pipe group is reduced duo to the reduction of stages in each epoch.
.hmacro BIU1_HM_un_s2
LPTO (1f ) @ (KI12); 
 BIU1.DM (A++,K++)->M[20] ; 
 BIU1.DM (A++,K++)->M[21] ; 
 BIU1.DM (A++,K++)->M[22] ; 
...
1:
 ...
 BIU1.DM (A++,K++)->M[34] ; 
 BIU1.DM (A++,K++)->M[35] ;  
 //The index of M shoulded be modified accordingto the group tail number
 // for 2048 float FFT, tail number = (2048/8) % PipeGroup = 8
Repeat @ (69-N); 
.endhmacro
...
////////////////////////////////////////////////////////////////////////////////
//The unbanlanced part for MR0, only one stage need.
.hmacro MR0_HM_un_s2
LPTO (1f ) @ (KI12); 
 r0.M[I++] ->SHU0.T0 || Repeat @ (20); 
repeat@(3);
...
///////////////////////////////////////////////////////////////////////////////
// The unbalanced MR2 
// The starting stage is 4 - TailStageNum
.hmacro MR2_HM_un_s2
LPTO (1f ) @ (KI12); 
//stage2
 r2.M[20] (WF2)->FMAC.T1 (I1) ; 
...
///////////////////////////////////////////////////////////////////////////////
// The unbalanced for SHU0
.macro SHU0_HM_un name Src1
.hmacro SHU0_HM_un_\name
LPTO (1f ) @ (KI12); 
 SHU0.T0 IND \Src1 ->FMAC.T0 (I0)|| Repeat @ (20); 
 repeat @ (3);
...
///////////////////////////////////////////////////////////////////////////////
// The unbalanced for SHU1
.macro SHU1_HM_un name Src1
.hmacro SHU1_HM_un_\name 
LPTO (1f ) @ (KI12); 
 SHU1.T0 IND \Src1 ->FALU.T0 (I2)|| Repeat @ (20); 
 repeat @ (3);
...
///////////////////////////////////////////////////////////////////////////////
// The unbalanced for FMAC
.hmacro FMAC_HM_un_s2
LPTO (1f ) @ (KI12); 
 T0 ** T1 (S)->FALU.T1 || Repeat @ (20); 
 repeat @ (3);
 T0 ** T1 (S)->FALU.T1 || Repeat @ (20); 
...
///////////////////////////////////////////////////////////////////////////////
// The unbalanced for FALU
.macro FALU_HM_un_  name BIUName
.hmacro FALU_HM_un\name
LPTO (1f ) @ (KI12); 
 T0 +- T1 (S)->M[0] ; 
 T0 +- T1 (S)->M[1] ; 
...
///////////////////////////////////////////////////////////////////////////////
// The unbalanced epoch
.hmacro main_hm_un
BIURead_un_biu2_read_s2 ;  //1
BIU1_HM_un_s2;
Repeat @ (10)     ;   //12
MR0_HM_un_s2;  //13
NOP;         
