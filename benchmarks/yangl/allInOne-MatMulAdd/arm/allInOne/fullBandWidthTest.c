...
static void printResult(int dataLength) {
	...
	{
		fetch_vector((uint32_t)(Result), SDA2DM0_START + offset, DMASeg,
				DT_BYTE, 0);
...
int fullBandWidthTest() {
	...
	*(volatile int*) 0x40401004 = 1;
	...
	for (dataLength = 0x100; dataLength <= MaxDataLength; dataLength <<= 1) {
		*(volatile int*) 0x40401000 = dataLength;
		...
		for (repeat_cnt = 0x10000; repeat_cnt <= 0x100000; repeat_cnt *= 4) {
			...
			*(volatile int*) 0x40401004 = (int) repeat_cnt;
			...
			if (time <= 0)
				printf(
					  "DataLength: 0x%x, 0x%llx executions, 0x%llx operations, total time: %lld ms, average time: %lld.%1d us, N.A. MOPS, bandwidth: N.A.\n",
					  dataLength, 
					  repeat_cnt, 
					  repeat_cnt * op, 
					  time,
				...
				printf(
					  "DataLength: 0x%x, 0x%llx executions, 0x%llx operations, total time: %lld ms, average time: %lld.%1d us, %lld.%1d MOPS, bandwidth: %lld KB/s\n",
					  dataLength, 
					  repeat_cnt, 
					  repeat_cnt * op, 
					  time,
