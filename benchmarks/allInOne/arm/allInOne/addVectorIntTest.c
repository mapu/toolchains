...
int addVectorIntTest() {
  ...
  trace("fill_src\n");
  send_vector(SDA0DM0_START, (const uint32_t) Src0, MaxDataLength, DT_WORD, 0);
  trace("send_vector\n");
  ape_dma_wait(DATA_E2I_GP, 0); //wait DMA to finish
  trace("ape_dma_wait\n");
  send_vector(SDA1DM0_START, (const uint32_t) Src1, MaxDataLength, DT_WORD, 0);
  trace("send_vector\n");
  ape_dma_wait(DATA_E2I_GP, 0); //wait DMA to finish
  trace("ape_dma_wait\n");
