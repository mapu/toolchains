...
int matmulTest(void) {
	...
	{
		send_vector(0x400000 + offset*4, (uint32_t) DM0Data, DMASeg, DT_WORD, 0);
		trace("send_vector 0x%x 0x%x\n", 0x400000 + offset, (uint32_t) DM0Data);
		ape_dma_wait(DATA_E2I_GP, 0); //wait DMA to finish
		trace("ape_dma_wait\n");
		send_vector(0x800000 + offset*4, (uint32_t) DM1Data, DMASeg, DT_WORD, 0);
		trace("send_vector 0x%x 0x%x\n", 0x800000 + offset, (uint32_t) DM1Data);
		ape_dma_wait(DATA_E2I_GP, 0); //wait DMA to finish
		trace("ape_dma_wait\n");
	}
  ...
  *mLength = 64;
  ...
  for (offset = 0; offset < *mLength * *mLength; offset += DMASeg)
	{
		fetch_vector((uint32_t)(DM2Data + offset * 4), 0xC00000 + offset*4, DMASeg, DT_WORD, 0);
		trace("fetch_vector 0x%x 0x%x\n", (uint32_t)(DM2Data), 0xC00000 + offset);
  ...
  for (*mLength = 64; *mLength <= MAX_LENGTH; *mLength*=2){
	  for (repeat_cnt = 0x1000; repeat_cnt <= 0x10000; repeat_cnt *= 4) {
	    ...
	    op = (*mLength)*(*mLength)*(*mLength *2 -1);
		  ...
		  if (time <= 0)
			  printf(
				    "length: %d, 0x%llx executions, 0x%llx operations, total time: %lld ms, average time: %lld.%1d us, N.A. MOPS\n",
				    *mLength, 
				    repeat_cnt, 
				    repeat_cnt * op, 
				    time, 
			  ...
			  printf(
				    "length: %d, 0x%llx executions, 0x%llx operations, total time: %lld ms, average time: %lld.%1d us, %lld.%1d MOPS\n",
				    *mLength,
				    repeat_cnt, 
				    repeat_cnt * op, 
				    time, 
