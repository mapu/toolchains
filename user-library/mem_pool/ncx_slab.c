...
#include "../../user-library/mem_pool/ncx_slab.h"
#include <unistd.h>
...
void ncx_slab_init(ncx_slab_pool_t *pool) {
  ...
  for (n = ncx_pagesize, ncx_pagesize_shift = 0; n >>= 1; ncx_pagesize_shift++) ;
...
ncx_slab_alloc_locked(ncx_slab_pool_t *pool, size_t size) {
  ...
  if (size >= ncx_slab_max_size) {
    ...
    page = ncx_slab_alloc_pages(pool, (size >> ncx_pagesize_shift) + ((size % ncx_pagesize) ? 1 : 0));
    if (page) {
