...
class mytest(unittest.TestCase): 
    ...
    def testDataShu(self):
        ...
        bytelist = ["","(b)","(h)","(l)","(tc)","(b,tc)","(h,tc)","(l,tc)"]
        for shu in shulist:
            for m in xrange(0,4):
                for n in xrange(0,4):
                    for byte in bytelist:
                        for biu in biulist:
                            for cond in condlist: 
                                text = "%s.comb(t%s,t%s)%s->%s%s"%(shu,m,n,byte,biu,cond)
                                #print text
            ...
            for sbh in sbhlist:
                for sbh in sbhlist:
                    for byte in bytelist:
                        for biu in biulist:
                            for cond in condlist:
                                text ="%s.comb(t%s,t%s)%s->%s%s"%(shu,sbh,sbh,byte,biu,cond)
                                self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
                                self.inittablewidget.dataParser(row, column)
                                self.assertEqual(self.inittablewidget.item(row, column).background(),self.inittablewidget.defaultBackgroundColor)
                                self.assertEqual(self.inittablewidget.database.searchMcc(self.inittablewidget.mmpulite.result), "3") 
                                for m in xrange(0,4):
                                    text0 = "%s.comb(t%s,t%s)%s->%s%s"%(shu,m,sbh,byte,biu,cond)
                                    text1 = "%s.comb(t%s,t%s)%s->%s%s"%(shu,sbh,m,byte,biu,cond)
                                    #print text0
                ...
                for n in xrange(0,4):
                    for byte in bytelist:
                        for t in xrange(0,127):
                            for cond in condlist: 
                                text = "%s.comb(t%s,t%s)%s->m[%s]%s"%(shu,m,n,byte,t,cond)
                                #print text
                                selranges = QTableWidgetSelectionRange(row, column, row, column )
                                self.inittablewidget.setRangeSelected(selranges, True)
                                self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
        ...
        for shu in shulist:
            for sbh in sbhlist:
                for byte in bytelist:
                    for t in xrange(0,127):
                        for cond in condlist:
                            text ="%s.comb(t%s,t%s)%s->m[%s]%s"%(shu,sbh,sbh,byte,t,cond)
                            self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
                            self.inittablewidget.dataParser(row, column)
                            self.assertEqual(self.inittablewidget.item(row, column).background(),self.inittablewidget.defaultBackgroundColor)
                            self.assertEqual(self.inittablewidget.database.searchMcc(self.inittablewidget.mmpulite.result), "4") 
                            for m in xrange(0,4):
                                text0 = "%s.comb(t%s,t%s)%s->m[%s]%s"%(shu,m,sbh,byte,t,cond)
                                text1 = "%s.comb(t%s,t%s)%s->m[%s]%s"%(shu,sbh,m,byte,t,cond)
                                #print text0
                                self.inittablewidget.setItem(row + 1, column,QTableWidgetItem(text0))
                                self.inittablewidget.setItem(row + 2, column,QTableWidgetItem(text1))
                                self.inittablewidget.dataParser(row + 1, column)
                    ...
                    for byte in bytelist:
                        for macc in macclist:
                            for t in xrange(0,4):
                                for i in ilist:
                                    for cond in condlist: 
                                        text = "%s.comb(t%s,t%s)%s->%s.t%s(%s)%s"%(shu,m,n,byte,macc,t,i,cond)
                                        #print text
                                        selranges = QTableWidgetSelectionRange(row, column, row, column )
                                        self.inittablewidget.setRangeSelected(selranges, True)
                                        self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
                    ...
                    for byte in bytelist:
                        for macc in macclist:
                            for t in xrange(0,4):
                                for i in ilist:
                                    for cond in condlist:
                                        text ="%s.comb(t%s,t%s)%s->%s.t%s(%s)%s"%(shu,sbh,sbh,byte,macc,t,i,cond)
                                        self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
                                        self.inittablewidget.dataParser(row, column)
                                        self.assertEqual(self.inittablewidget.item(row, column).background(),self.inittablewidget.defaultBackgroundColor)
                                        self.assertEqual(self.inittablewidget.database.searchMcc(self.inittablewidget.mmpulite.result), "3") 
                                        for m in xrange(0,4):
                                            text0 = "%s.comb(t%s,t%s)%s->%s.t%s(%s)%s"%(shu,m,sbh,byte,macc,t,i,cond)
                                            text1 = "%s.comb(t%s,t%s)%s->%s.t%s(%s)%s"%(shu,sbh,m,byte,macc,t,i,cond)
                                            #print text0
                                            self.inittablewidget.setItem(row + 1, column,QTableWidgetItem(text0))
                                            self.inittablewidget.setItem(row + 2, column,QTableWidgetItem(text1))
                                            self.inittablewidget.dataParser(row + 1, column)
            ...
            for m in xrange(0,4):
                for n in xrange(0,4):
                    for byte in bytelist:
                        for t in xrange(0,4):
                            for cond in condlist: 
                                text = "%s.comb(t%s,t%s)%s->%s.t%s%s"%(shu,m,n,byte,shu,t,cond)
                                #print text
                                selranges = QTableWidgetSelectionRange(row, column, row, column )
                                self.inittablewidget.setRangeSelected(selranges, True)
                                self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
            ...
            for sbh in sbhlist:
                for sbh in sbhlist:
                    for byte in bytelist:
                        for t in xrange(0,4):
                            for cond in condlist:
                                text ="%s.comb(t%s,t%s)%s->%s.t%s%s"%(shu,sbh,sbh,byte,shu,t,cond)
                                self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
                                self.inittablewidget.dataParser(row, column)
                                self.assertEqual(self.inittablewidget.item(row, column).background(),self.inittablewidget.defaultBackgroundColor)
                                self.assertEqual(self.inittablewidget.database.searchMcc(self.inittablewidget.mmpulite.result), "2") 
                                for m in xrange(0,4):
                                    text0 = "%s.comb(t%s,t%s)%s->%s.t%s%s"%(shu,m,sbh,byte,shu,t,cond)
                                    text1 = "%s.comb(t%s,t%s)%s->%s.t%s%s"%(shu,sbh,m,byte,shu,t,cond)
                                    print text
                                    self.inittablewidget.setItem(row + 1, column,QTableWidgetItem(text0))
                                    self.inittablewidget.setItem(row + 2, column,QTableWidgetItem(text1))
                                    self.inittablewidget.dataParser(row + 1, column)
