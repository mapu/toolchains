...
class mytest(unittest.TestCase): 
                ...
                for byte in bytelist:
                    for biu in biulist:
                        for cond in condlist:
                            text = "t%s* t%s(%s)->%s%s"%(m,n,byte,biu,cond)
                            #print text
            ...
            for n in xrange(0,4):
                for byte in bytelist:
                    for t in xrange(0,127):
                        for cond in condlist:
                            text = "t%s* t%s(%s)->m[%s]%s"%(m,n,byte,t,cond)
                            #print text
                            selranges = QTableWidgetSelectionRange(row, column, row, column)
                            self.inittablewidget.setRangeSelected(selranges, True)
                            self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
            ...
            for n in xrange(0,4):
                for byte in bytelist:
                    for macc in macclist:
                        for t in xrange(0,4):
                            for cond in condlist:
                                text = "t%s* t%s(%s)->%s.t%s%s"%(m,n,byte,macc,t,cond)
                                #print text
                                selranges = QTableWidgetSelectionRange(row, column, row, column)
                                self.inittablewidget.setRangeSelected(selranges, True)
                                self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
            ...
            for n in xrange(0,4):
                for byte in bytelist:
                    for shu in shulist:
                        for t in xrange(0,4):
                            for cond in condlist:
                                text = "t%s* t%s(%s)->%s.t%s%s"%(m,n,byte,shu,t,cond)
                                #print text
                                selranges = QTableWidgetSelectionRange(row, column, row, column)
                                self.inittablewidget.setRangeSelected(selranges, True)
                                self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
                    ...
                    for macc in macclist:
                        for t in xrange(0,4):
                            for cond in condlist:
                                ...
                                self.inittablewidget.dataParser(row, column)
                                self.assertEqual(self.inittablewidget.item(row, column).background(),self.inittablewidget.defaultBackgroundColor)
                                self.assertEqual(self.inittablewidget.database.searchMcc(self.inittablewidget.mmpulite.result), "10")
        ...
        for p in xrange(0,4):
            for m in xrange(0,4):
                for n in xrange(0,4):
                    for byte in bytelist:
                        for cond in condlist:
                            text = "mr<-t%s + t%s* t%s(%s)%s"%(p,m,n,byte,cond)
                            #print text
                            selranges = QTableWidgetSelectionRange(row, column, row, column)
                            self.inittablewidget.setRangeSelected(selranges, True)
                            self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
    ...
    def testFmac_8(self):
        ...
        row = random.randint(0,1999)
        column = random.randint(0,19)
        bytelist = ["s","d","cr,s","cr,d"]
        for m in xrange(0,4):
            for n in xrange(0,4):
                for byte in bytelist:
                    for cond in condlist:
                        text = "mr += t%s* t%s(%s)%s"%(m,n,byte,cond)
                        #print text
                        selranges = QTableWidgetSelectionRange(row, column, row, column)
                        self.inittablewidget.setRangeSelected(selranges, True)
                        self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
        ...
        for m in xrange(0,4):
            for n in xrange(0,4):
                for byte in bytelist:
                    for biu in biulist:
                        for cond in condlist:
                            text = "mr += t%s* t%s(%s)->%s%s"%(m,n,byte,biu,cond)
                            #print text
                            selranges = QTableWidgetSelectionRange(row, column, row, column)
                            self.inittablewidget.setRangeSelected(selranges, True)
                            self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
        ...
        for m in xrange(0,4):
            for n in xrange(0,4):
                for byte in bytelist:
                    for t in xrange(0,127):
                        for cond in condlist:
                            text = "mr += t%s* t%s(%s)->m[%s]%s"%(m,n,byte,t,cond)
                            #print text
                            selranges = QTableWidgetSelectionRange(row, column, row, column)
                            self.inittablewidget.setRangeSelected(selranges, True)
                            self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
            ...
            for n in xrange(0,4):
                for byte in bytelist:
                    for macc in macclist:
                        for t in xrange(0,4):
                            for cond in condlist:
                                text = "mr += t%s* t%s(%s)->%s.t%s%s"%(m,n,byte,macc,t,cond)
                                #print text
                                selranges = QTableWidgetSelectionRange(row, column, row, column)
                                self.inittablewidget.setRangeSelected(selranges, True)
                                self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
            ...
            for n in xrange(0,4):
                for byte in bytelist:
                    for shu in shulist:
                        for t in xrange(0,4):
                            for cond in condlist:
                                text = "mr += t%s* t%s(%s)->%s.t%s%s"%(m,n,byte,shu,t,cond)
                                #print text
                                selranges = QTableWidgetSelectionRange(row, column, row, column)
                                self.inittablewidget.setRangeSelected(selranges, True)
                                self.inittablewidget.setItem(row, column,QTableWidgetItem(text))
