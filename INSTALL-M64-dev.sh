...
then
 ...
 if [ ! -e "build_ragel" ]
  ...
  cd build_llvm
  cmake $source_path/llvm-3.8-M64-dev -DLLVM_TARGETS_TO_BUILD="X86;MSPU;MMPULite;UCPM;UCPS" -DCMAKE_C_FLAGS="-DARCH_MAPU" -DCMAKE_CXX_FLAGS="-DARCH_MAPU" -DCOMPILER_RT_DEFAULT_TARGET_TRIPLE="mspu-unknown-unknown" -DLLVM_BUILD_EXTERNAL_COMPILER_RT=On -DCMAKE_BUILD_TYPE=$llvm_cfg -DCMAKE_INSTALL_PREFIX=$install_path -DRAGEL=$root/ragel/bin/ragel -DLLVM_LIBDIR_SUFFIX=64
