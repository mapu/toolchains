...
static struct mapu_dev *mapu_lookfor_device(pid_t key) {
  ...
  list_for_each_entry(lptr, &mapu_list, list) {
    ...
    if (lptr->u_key == key || lptr->m_key == key) {
      if (lptr->u_key == key) lptr->urefcnt++;
      return &(lptr->device);
    }
  }
...
int mapu_ioctl(struct inode *inode, struct file *filp,
  ...
  if (_IOC_TYPE(cmd) != MAPU_IOC_MAGIC) return -ENOTTY;
...
int mapu_init(void) {
  ...
  for (i = 0; i < MAX_NUM_PAIRS; i++) {
    share_mem_pool[i] = alloc_pages(GFP_KERNEL | __GFP_HIGHMEM | __GFP_REPEAT, 10);
    if (!share_mem_pool[i]) {
    ...
    for (j = 0; j < MAX_NUM_PAIRS; j++) {
      ddr_pool[i][j] = alloc_pages(GFP_KERNEL | __GFP_HIGHMEM | __GFP_REPEAT, 10);
      if (!ddr_pool[i][j]) {
