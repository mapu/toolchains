...
void FixPointTurboDecoder::Decoder::gamma() {
  ...
  for (unsigned i = 0; i < 8; i++)
    DPRINTFR(TurboDecBeta, " %d(%#08x)", t_beta[i], *(metric_t *)&t_beta[i]);

  DPRINTFR(TurboDecBeta, " @%#02x\n", pos);
#endif
  if (pos < cur_beta_len)
...
void FixPointTurboDecoder::Decoder::compute_llr(state_metric b, sys_t systematic) {
  ...
  DPRINTF(TurboDecLLR, "[iter: %d, id: %d, sw: %d, pos: %d] llr(): llr: %d(%#08x) "
          ...
          *(metric_t *)&t_llr, t_le, *(metric_t *)&t_le, cur_pos);
}
...
void FixPointTurboDecoder::Decoder::reset() {
  parent->dec_res[id].cur_sys_base = parent->conf_regs.p_sys_base +
        parent->conf_regs.p_sb_len * sizeof(sys_t) * id;
...
FixPointTurboDecoder::~FixPointTurboDecoder() {
  for (unsigned i = 0; i < NUM_SUB_DEC; i++) {
    ...
    if (dec_res[i].interleaver)
      delete dec_res[i].interleaver;
  }
}
    ...
    if (current_iter < conf_regs.p_max_interation_num) {
      ...
      for (unsigned i = 0; i < conf_regs.p_sb_num; i++) {
        dec_res[i].decoder->setload(cur_sw < conf_regs.p_sw_num - 1 ?
        		                conf_regs.p_sw_len_1 : conf_regs.p_sw_len_2);
        pending_status.decode = pending_status.decode | (1 << i);
        assert(!dec_res[i].decoder->tickEvent.scheduled());
        schedule(dec_res[i].decoder->tickEvent, curTick());
      }
...
Tick FixPointTurboDecoder::write(PacketPtr pkt) {
  ...
  if (daddr == 0 && !decodeEvent.scheduled()) {
    status = 1;
    current_iter = 0;
    current_state = DEC_STATES[0];
    pending_status = 0;
  ...
  case TURBODEC_SYS_BASE:
    ...
    default:
      if (daddr >= TURBODEC_BETA_INIT_1 && daddr < TURBODEC_BETA_INIT_2)
        conf_regs.p_beta_initial_value_cb1[(daddr - TURBODEC_BETA_INIT_1) / 4] = data;
      else if (daddr >= TURBODEC_BETA_INIT_2 && daddr < TURBODEC_INTLV_CONF0)
        conf_regs.p_beta_initial_value_cb2[(daddr - TURBODEC_BETA_INIT_2) / 4] = data;
      else {
