...
class DataBase():
	...
	self.snMTableName="snMPU"
	self.snSTableName="snSPU"
	...
	i=datetime.datetime.now()
        print ("start mem %s:%s:%s,%s" %(i.hour,i.minute,i.second,i.microsecond))
	self.progressCall(self.value,"start bulid mem table",-1)
	self.memTableInit()
	self.progressCall(self.value,"end build mem table",-1)
	i=datetime.datetime.now()
        print ("start ape0 sn reg %s:%s:%s,%s" %(i.hour,i.minute,i.second,i.microsecond))
...
#129000[64]: system.cpu.graduation_unit: [tid:1]: [sn:25]: [sln:25]: 0x000047: (memo:biu0ld    ) BIU0.DM(A++,K++)->M[I++] : 109000 110000 111000 112000 113000 114000 115000 116000 117000 118000 120000 122000 124000 125000 126000 127000 128000 129000
    ...
    def APESnRegTableInit(self,APEdbConn,n): 
        #drop snM table
	...
	item_sn=SnItem()
        ...
        lines=f.readlines()
        for line in lines:
	    if line.find(s0)>=0 or line.find(s1)>=0:
	...
	if 0:
        ...
        #show snS table
	if 0:
            fetchall_sql = "SELECT * FROM "+self.snSTableName
            cu = self.get_cursor(APEdbConn)
            cu.execute(fetchall_sql)
            r = cu.fetchall()
            if len(r) > 0:
	...
	for i in range(1,len(timeList)):
	    ...
	    self.value+=self.maxValue*0.1*p
	    self.progressCall(self.value,"building time table",-1)
