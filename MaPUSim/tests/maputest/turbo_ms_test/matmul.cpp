...
CsuObject *sim_init() {
  ...
  std::string spuname = "/home/wujn/Simulator/"
    "SDK/sdk/template/turbo_test_fix/slib/bin/turbo_test_fix.s.elf";
...
int main() {
  ...
  for (en = 0; en < EbN0_NUM; en++) {
    ...
    for (i = 0; i < blk_num; i++) {
      ...
      for (j = 0; j < blk_len + 4; j++) {
        dec_bits_sys_fix[j] = fixed_point(sys_llr[j], 6, 2);
        dec_bits_par1_fix[j] = fixed_point(parity1_llr[j], 6, 2);
        dec_bits_par2_fix[j] = fixed_point(parity2_llr[j], 6, 2);
      }
        //LOG FILE
        //   for(j=0;j<blk_len;j++)
        //   {
...
char
fixed_point(float input, int numBit, int p)
{
  char result;
  if(input * (float)( 1 << p) >= 31.0)
    result = 31;
  else if (input *(float)(1 << p) <= -32.0)
    result = -32;
  else
    result = (char) floor(input * (float)(1 << p) ); //quantization
