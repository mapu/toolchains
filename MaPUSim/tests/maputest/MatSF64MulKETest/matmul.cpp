...
int main() {
  ...
  conf.DMAGlobalAddr = 0x70000000;
  conf.DMAGlobalXNum = 4 * 67;
  conf.DMAGlobalYAllNum = 65;
  conf.DMAGlobalYNum = 16;
  conf.DMAGlobalYStep = 4 * 67;
  conf.DMAGlobalZStep = 4 * 67 * 16;
  conf.DMALocalAddr = 0xc00000;
  conf.DMALocalXNum = 4 * 67;
  conf.DMALocalYAllNum = 65;
  conf.DMALocalYNum = 16;
  conf.DMALocalYStep = 4096 * 4;
  conf.DMALocalZStep = 4 * 67;
  conf.DMAGroupNum = 3;
  conf.DMACmd = 1;
  ...
  std::fstream a("../m5out/MatSF64MulKETest/MatSF64MulKE.out", std::ios::out | std::ios::trunc);
  ...
  while (in && i < 4355) {
	  ...
	  if (code != result[i++])
		   a << std::hex << code << " : " << result[i-1] << std::endl;
  }
