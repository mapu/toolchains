...
UcpLiveProcess::UcpLiveProcess(LiveProcessParams * params,
        ObjectFile *objFile)
    : LiveProcess(params, objFile)
{
    // Set up stack. On Ucp, stack starts at the top of kuseg
    // user address space. Ucp stack grows down from here
    stack_base = 0x20000;
...
void
UcpLiveProcess::argsInit(int pageSize)
{
    ...
    if (elfObject)
    {
        ...
        auxv.push_back(auxv_t(M5_AT_PAGESZ, UcpISA::VMPageSize));
...
UcpISA::IntReg
UcpLiveProcess::getSyscallArg(ThreadContext *tc, int &i)
{
    assert(i < 6);
...
void
UcpLiveProcess::setSyscallArg(ThreadContext *tc,
...
void
UcpLiveProcess::setSyscallReturn(ThreadContext *tc,
