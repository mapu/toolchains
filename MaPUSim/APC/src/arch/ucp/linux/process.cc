...
SyscallDesc UcpSPUProcess::syscallDescs[] = {
    /*  0 */ SyscallDesc("syscall", unimplementedFunc),
    /*  1 */ SyscallDesc("exit", exitFunc),
    /*  2 */ SyscallDesc("fork", unimplementedFunc),
    /*  3 */ SyscallDesc("read", readFunc),
    /*  4 */ SyscallDesc("write", writeFunc),
    /*  5 */ SyscallDesc("open", openFunc<UcpLinux>),
    ...
    /* 15 */ SyscallDesc("chmod", chmodFunc<UcpLinux>),
    ...
    /* 54 */ SyscallDesc("ioctl", ioctlFunc<UcpLinux>),
    ...
    /* 77 */ SyscallDesc("getrusage", getrusageFunc<UcpLinux>),
    ...
    /* 106 */ SyscallDesc("stat",  statFunc<UcpLinux>),
    /* 107 */ SyscallDesc("lstat", unimplementedFunc),
    /* 108 */ SyscallDesc("fstat", fstatFunc<UcpLinux>),
    ...
    /* 116 */ SyscallDesc("sysinfo", sysinfoFunc<UcpLinux>),
    ...
    /* 167 */ SyscallDesc("mremap", mremapFunc<UcpLinux>),
    ...
    /* 214 */ SyscallDesc("lstat64", lstat64Func<UcpLinux>),
    /* 215 */ SyscallDesc("fstat64", fstat64Func<UcpLinux>),
    ...
    /* 283 */ SyscallDesc("set_thread_area", unimplementedFunc/*setThreadAreaFunc*/),
...
UcpSPUProcess::UcpSPUProcess(LiveProcessParams * params,
                                     ObjectFile *objFile)
    : UcpLiveProcess(params, objFile), block(false),
     Num_Syscall_Descs(sizeof(syscallDescs) / sizeof(SyscallDesc))
{  }

SyscallDesc*
UcpSPUProcess::getDesc(int callnum)
{
    //MaPU syscalls are in the range of 0 - 1024
...
UcpMPUProcess::UcpMPUProcess(LiveProcessParams * params,
                                     ObjectFile *objFile)
    : LiveProcess(params, objFile)
{  }

UcpISA::IntReg
UcpMPUProcess::getSyscallArg(ThreadContext *tc, int &i)
{
	//assert(0 && "MPU doesn't support syscall\n");
    return 0;
}

void
UcpMPUProcess::setSyscallArg(ThreadContext *tc,
        int i, UcpISA::IntReg val)
{
	//assert(0 && "MPU doesn't support syscall\n");
}

void
UcpMPUProcess::setSyscallReturn(ThreadContext *tc,
...
SyscallDesc*
UcpMPUProcess::getDesc(int callnum)
{
	if (callnum == 246) return &UcpSPUProcess::syscallDescs[246];
	assert(0 && "MPU doesn't support syscall\n");
    return NULL;
}
...
UcpMPUProcess::initState()
{
    ...
    // mark this MPU thread context as halted so it won't start
    // ticking until we call CALLM.
    tc->halt(0);
