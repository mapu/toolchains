#ifndef __ARCH_UCP_TYPES_HH__
#define __ARCH_UCP_TYPES_HH__
...
class MaPUCPState: public GenericISA::VLIWPCState<MachInst> {
protected:
...
public:
  ...
  // Advance the PC.
  void madvance() {
    const unsigned slotSize[UcpISA::NumSlots] = { 23, 20, 21, 15, 28, 28, 31,
                                                   29, 26, 24, 20, 20, 20, 23 };
    Base::_pc = Base::_npc;
    _pcb = _npcb;
    slotNum = (slotNum + 1) % UcpISA::NumSlots;
    Base::_nlpc += (slotNum == 0) ? 41 : 0;
    _npcb += slotSize[slotNum];
    Base::_npc = _npcb / 8;
  }

  bool operator ==(const MaPUCPState<MachInst> &opc) const {
...
template<class MachInst>
std::ostream &
operator<<(std::ostream & os, const MaPUCPState<MachInst> &pc) {
  ccprintf(os, "(%#x=>%#x:%#x)", pc.pc(), pc.npc(), pc.nlpc());
  return os;
}

typedef MaPUCPState<MachInst> PCState;
