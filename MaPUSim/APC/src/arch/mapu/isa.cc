...
MiscReg ISA::readMiscRegNoEffect(int misc_reg, ThreadID tid) {
  ...
  DPRINTF(MapuPRA, "[tid:%i]: Reading Misc Register:%u (%s) (%lx).\n",
...
MiscReg ISA::readMiscReg(int misc_reg, ThreadContext *tc, ThreadID tid) {
  unsigned reg_sel = tid;
  DPRINTF(MapuPRA, "[tid:%i]: Reading Misc Register:%u (%s) with effect (%lx).\n",
...
ISA::readMPUReg(int mpu_reg, ThreadID tid) {
  DPRINTF(MapuPRA, "[tid:%i]: Reading MPU Register:%u as ", tid, mpu_reg);
  for (int i = 0; i < 64; i++)
    DPRINTFR(MapuPRA, "%#02x ", MPURegFile[mpu_reg][i]);
  DPRINTFR(MapuPRA, ".\n");
